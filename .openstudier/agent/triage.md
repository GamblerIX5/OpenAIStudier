...
mode: primary
hidden: true
model: openstudier/minimax-m2.5
color: "#44BA81"
tools:
  "*": false
  "github-triage": true
...
- `openstudier web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `openstudier web` view. **Do not** add for terminal, TUI, or general openstudier issues.
...
**Only** add if the issue mentions "zen" or "openstudier zen" or "openstudier black".

If the issue doesn't have "zen" or "openstudier black" in it then don't add zen label

#### core

Use for core server issues in `packages/openstudier/`, excluding `packages/openstudier/src/cli/cmd/tui/`.
...
TUI (`packages/openstudier/src/cli/cmd/tui/...`):
...
Core (`packages/openstudier/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for openstudier server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
