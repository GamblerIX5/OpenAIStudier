...
async function terminalReady(page: Page, term?: Locator) {
  ...
  return page.evaluate((id) => {
    const state = (window as E2EWindow).__openstudier_e2e?.terminal?.terminals?.[id]
...
async function terminalHas(page: Page, input: { term?: Locator; token: string }) {
  ...
  return page.evaluate(
    (input) => {
      const state = (window as E2EWindow).__openstudier_e2e?.terminal?.terminals?.[input.id]
...
export async function seedProjects(page: Page, input: { directory: string; extra?: string[] }) {
  await page.addInitScript(
    (args: { directory: string; serverUrl: string; extra: string[] }) => {
      const key = "openstudier.global.dat:server"
...
export async function createTestProject() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "openstudier-e2e-project-"))
