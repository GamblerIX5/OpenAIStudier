...
import { type Session } from "@openstudier/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openstudier://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openstudier://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openstudier://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openstudier://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openstudier://open-project")).toBeUndefined()
    expect(parseDeepLink("openstudier://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openstudier://open-project?directory=/a",
      "openstudier://other?directory=/b",
      "openstudier://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openstudier://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openstudier://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openstudier://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openstudier://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openstudier://new-session?directory=/a",
      "openstudier://open-project?directory=/b",
      "openstudier://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENSTUDIER__: {
        deepLinks: ["openstudier://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENSTUDIER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openstudier://open-project?directory=/a"])
