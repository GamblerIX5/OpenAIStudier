...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.openstudier.desktop.dev",
  beta: "ai.openstudier.desktop.beta",
  prod: "ai.openstudier.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.openstudier.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openstudier.settings.dat` is special: it maps to the `openstudier.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"openstudier.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // openstudier.settings.dat → the electron settings store ("openstudier.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "openstudier.global.dat").
  const storeName = filename === "openstudier.settings.dat" ? "openstudier.settings" : filename
