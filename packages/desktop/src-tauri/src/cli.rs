...
const CLI_INSTALL_DIR: &str = ".openstudier/bin";
const CLI_BINARY_NAME: &str = "openstudier";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("openstudier-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "OPENSTUDIER_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "OPENSTUDIER_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("OPENSTUDIER_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.openstudier/bin/openstudier\"".to_string(),
            ...
            let mut env_prefix = vec![
                "OPENSTUDIER_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "OPENSTUDIER_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "OPENSTUDIER_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "OPENSTUDIER_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "OPENSTUDIER_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "OPENSTUDIER_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("OPENSTUDIER_SERVER_USERNAME", "openstudier".to_string()),
        ("OPENSTUDIER_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn openstudier");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("OPENSTUDIER_CLIENT"), Some(&"desktop".to_string()));
    }
