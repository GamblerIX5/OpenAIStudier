...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("openstudier-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("openstudier_lib=debug,openstudier_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("openstudier_lib=info,openstudier_desktop=info,sidecar=info")
        }
    });
