...
Replace runtime Bun shell template-tag usage in `packages/openstudier/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openstudier/src` is removed except:
