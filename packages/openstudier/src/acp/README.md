...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openstudier.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openstudier sessions
...
openstudier acp --cwd /path/to/project
```
...
```bash
OPENSTUDIER_ENABLE_QUESTION_TOOL=1 openstudier acp
```
...
- **Terminal Integration**: Full terminal support via openstudier's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openstudier acp
```
...
### Mapping to OpenStudier

ACP sessions map cleanly to openstudier's internal session model:
