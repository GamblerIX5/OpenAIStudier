...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@openstudier/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `openstudier auth login` in the terminal",
        name: "Login with openstudier",
        id: "openstudier-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // OpenStudier stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const openstudierProvider = providers.find((p) => p.id === "openstudier")
    if (openstudierProvider) {
      if (openstudierProvider.models["big-pickle"]) {
        return { providerID: ProviderID.openstudier, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(openstudierProvider.models))
      if (best) {
        return {
