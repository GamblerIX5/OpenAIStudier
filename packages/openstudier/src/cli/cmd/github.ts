...
const AGENT_USERNAME = "openstudier-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/openstudier.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/openstudier-agent"
            const command =
...
jobs:
  openstudier:
    if: |
      ...
      startsWith(github.event.comment.body, '/openstudier')
    ...
    steps:
      ...
      - name: Run openstudier
        uses: GamblerIX/OpenStudier/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("openstudier session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/openstudier,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to openstudier...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("openstudier-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `openstudier/${type}-${hex}-${timestamp}`
        }
        return `openstudier/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/openstudier-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[openstudier session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the openstudier infrastructure after your response",
