...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run openstudier",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for openstudier session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found openstudier session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["openstudier", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting openstudier...")
        UI.println()

        // Launch openstudier TUI with session ID if available
        const { spawn } = await import("child_process")
        const openstudierArgs = sessionId ? ["-s", sessionId] : []
        const openstudierProcess = spawn("openstudier", openstudierArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          openstudierProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`openstudier exited with code ${code}`))
          })
          openstudierProcess.on("error", reject)
        })
      },
    })
  },
