...
const TIPS = [
  ...
  "Create {highlight}openstudier.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/openstudier/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "OpenStudier auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.openstudier/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.openstudier/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "OpenStudier auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "OpenStudier uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.openstudier/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.openstudier/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent OpenStudier from reading sensitive files",
  "Use {highlight}openstudier run{/highlight} for non-interactive scripting",
  "Use {highlight}openstudier --continue{/highlight} to resume the last session",
  "Use {highlight}openstudier run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}openstudier serve{/highlight} for headless API access to OpenStudier",
  "Use {highlight}openstudier run --attach{/highlight} to connect to a running server",
  "Run {highlight}openstudier upgrade{/highlight} to update to the latest version",
  "Run {highlight}openstudier auth list{/highlight} to see all configured providers",
  "Run {highlight}openstudier agent create{/highlight} for guided agent creation",
  "Use {highlight}/openstudier{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}openstudier github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/openstudier fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.openstudier/themes/{/highlight} directory",
  ...
  "Run {highlight}openstudier debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/gamblerix/openstudier{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with OpenStudier Zen for curated, tested models",
