...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openstudier and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenStudier")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openstudier",
      pnpm: "pnpm uninstall -g openstudier",
      bun: "bun remove -g openstudier",
      yarn: "yarn global remove openstudier",
      brew: "brew uninstall openstudier",
      choco: "choco uninstall openstudier",
      scoop: "scoop uninstall openstudier",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openstudier", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".openstudier")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenStudier!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openstudier") || content.includes(".openstudier/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openstudier/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".openstudier/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".openstudier"))
    ) {
      continue
    }
