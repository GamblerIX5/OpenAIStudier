...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openstudier.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openstudier = await openstudierFiles(input)
  for (const file of openstudier) {
...
async function openstudierFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENSTUDIER_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("openstudier", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "openstudier")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openstudier"))
  }
  if (Flag.OPENSTUDIER_CONFIG) files.push(Flag.OPENSTUDIER_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "openstudier"))
