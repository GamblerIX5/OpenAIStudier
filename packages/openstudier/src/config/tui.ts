...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.OPENSTUDIER_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.OPENSTUDIER_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.OPENSTUDIER_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".openstudier") && dir !== Flag.OPENSTUDIER_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old openstudier.json shape) still get their settings applied.
    const normalized = (() => {
