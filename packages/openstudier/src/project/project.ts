...
import { fn } from "@openstudier/util/fn"
...
export namespace Project {
  ...
  function readCachedId(dir: string) {
    return Filesystem.readText(path.join(dir, "openstudier"))
      .then((x) => x.trim())
      .then(ProjectID.make)
      .catch(() => undefined)
  }
  ...
  export async function fromDirectory(directory: string) {
      ...
      if (dotgit) {
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox,
            vcs: Info.shape.vcs.parse(Flag.OPENSTUDIER_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          if (id) {
            await Filesystem.write(path.join(dotgit, "openstudier"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.OPENSTUDIER_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
