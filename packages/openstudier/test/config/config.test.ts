...
const managedConfigDir = process.env.OPENSTUDIER_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openstudier.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openstudier.json") {
...
test("ignores legacy tui keys in openstudier config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "openstudier.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Filesystem.write(
          path.join(dir, "openstudier.json"),
          JSON.stringify({
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openstudier.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["OPENSTUDIER_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      openstudier: {
        options: {
          apiKey: "{env:OPENSTUDIER_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["openstudier"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["OPENSTUDIER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENSTUDIER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openstudier.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openstudier directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openstudierDir = path.join(dir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })
      const agentDir = path.join(openstudierDir, "agent")
...
test("loads agents from .openstudier/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openstudierDir = path.join(dir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })

      const agentsDir = path.join(openstudierDir, "agents")
...
test("loads commands from .openstudier/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openstudierDir = path.join(dir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })

      const commandDir = path.join(openstudierDir, "command")
...
test("loads commands from .openstudier/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openstudierDir = path.join(dir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })

      const commandsDir = path.join(openstudierDir, "commands")
...
test("does not try to install dependencies in read-only OPENSTUDIER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENSTUDIER_CONFIG_DIR
  process.env.OPENSTUDIER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENSTUDIER_CONFIG_DIR
    else process.env.OPENSTUDIER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENSTUDIER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "openstudier.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openstudier config
      const projectDir = path.join(dir, "project")
      const openstudierDir = path.join(projectDir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })
      ...
      // Local .openstudier config with different plugins
      await Filesystem.write(
        path.join(openstudierDir, "openstudier.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openstudier config with some overlapping plugins
      await Filesystem.write(
        path.join(openstudierDir, "openstudier.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets OPENSTUDIER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openstudier config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openstudier directory config enables it
      const openstudierDir = path.join(dir, ".openstudier")
      await fs.mkdir(openstudierDir, { recursive: true })
      await Filesystem.write(
        path.join(openstudierDir, "openstudier.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/openstudier")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/openstudier")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-openstudier@2.4.3")).toBe("oh-my-openstudier")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@openstudier/plugin@2.0.0")).toBe("@openstudier/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-openstudier@2.4.3", "file:///project/.openstudier/plugin/oh-my-openstudier.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openstudier/plugin/oh-my-openstudier.js")
  })
  ...
  test("local plugin directory overrides global openstudier.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const openstudierDir = path.join(projectDir, ".openstudier")
        const pluginDir = path.join(openstudierDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENSTUDIER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"]
    process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openstudier/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"]
    process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openstudier directory with a command
          const openstudierDir = path.join(dir, ".openstudier", "command")
          await fs.mkdir(openstudierDir, { recursive: true })
          await Filesystem.write(path.join(openstudierDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .openstudier should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENSTUDIER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENSTUDIER_CONFIG_DIR"]
      process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENSTUDIER_CONFIG_DIR"]
      } else {
        process.env["OPENSTUDIER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENSTUDIER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENSTUDIER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENSTUDIER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENSTUDIER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENSTUDIER_CONFIG_DIR, not project
...
describe("OPENSTUDIER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENSTUDIER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENSTUDIER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENSTUDIER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENSTUDIER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENSTUDIER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENSTUDIER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENSTUDIER_CONFIG_CONTENT"]
