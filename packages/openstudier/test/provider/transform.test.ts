...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const openstudierModel = {
      ...openaiModel,
      providerID: "openstudier",
      api: {
        id: "openstudier-test",
    ...
    const result = ProviderTransform.message(msgs, openstudierModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.openstudier?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.openstudier?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          openstudier: { itemId: "msg_openstudier" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              openstudier: { itemId: "msg_openstudier_part" },
    ...
    expect(result[0].providerOptions?.openstudier?.itemId).toBe("msg_openstudier")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.openstudier?.itemId).toBe("msg_openstudier_part")
