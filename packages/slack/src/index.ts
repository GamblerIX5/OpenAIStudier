...
import { createOpencode, type ToolPart } from "@openstudier/sdk"
...
console.log("🚀 Starting openstudier server...")
const openstudier = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await openstudier.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openstudier session...")
    const { client, server } = openstudier
    ...
    console.log("✅ Created openstudier session:", createResult.data.id)
  ...
  console.log("📝 Sending to openstudier:", message.text)
