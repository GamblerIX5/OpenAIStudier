...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("openstudier-find")
  api.delete("openstudier-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("openstudier-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("openstudier-find", new Highlight(...rest))
    return true
  }
