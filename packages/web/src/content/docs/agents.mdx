...
There are two types of agents in OpenStudier; primary agents and subagents.
...
OpenStudier comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenStudier comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenStudier comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `openstudier.json` config file:
...
- Per-project: `.openstudier/agents/`
...
If no temperature is specified, OpenStudier uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenStudier config and the project specific config.
...
The model ID in your OpenStudier config uses the format `provider/model-id`. For example, if you're using [OpenStudier Zen](/docs/zen), you would use `openstudier/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `openstudier models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/GamblerIX/OpenStudier).
:::
