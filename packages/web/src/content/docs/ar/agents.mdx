...
يوجد نوعان من الوكلاء في OpenStudier: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي OpenStudier مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي OpenStudier مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي OpenStudier مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `openstudier.json`:
...
- عام: `~/.config/openstudier/agents/`
- لكل مشروع: `.openstudier/agents/`
...
إذا لم يتم تحديد temperature، يستخدم OpenStudier القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات OpenStudier العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات OpenStudier الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [OpenStudier Zen](/docs/zen)، فستستخدم `openstudier/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `openstudier models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/GamblerIX/OpenStudier).
:::
