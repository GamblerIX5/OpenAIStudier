...
أنشئ الملف `.openstudier/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenStudier أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenStudier:
...
- على مستوى النظام: `~/.config/openstudier/commands/`
- لكل مشروع: `.openstudier/commands/`
...
يتضمن openstudier عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
