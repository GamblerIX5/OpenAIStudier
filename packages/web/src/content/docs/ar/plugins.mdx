---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع OpenStudier.
---

تتيح لك الإضافات توسيع OpenStudier عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ OpenStudier.
...
- `.openstudier/plugins/` - إضافات على مستوى المشروع
- `~/.config/openstudier/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/openstudier/node_modules/`.
...
1. إعدادات عامة (`~/.config/openstudier/openstudier.json`)
2. إعدادات المشروع (`openstudier.json`)
3. دليل الإضافات العام (`~/.config/openstudier/plugins/`)
4. دليل إضافات المشروع (`.openstudier/plugins/`)
...
يشغّل OpenStudier الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ openstudier للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع openstudier.
...
:::note
إذا كنت تستخدم تطبيق OpenStudier لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع openstudier من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى openstudier:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ openstudier استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ openstudier إلى جانب الأدوات المضمنة.
