---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenStudier ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenStudier.
...
في `~/.local/share/openstudier/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenStudier.
...
OpenStudier Zen هي قائمة نماذج يوفّرها فريق OpenStudier وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenStudier. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenStudier Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر openstudier، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenStudier واستخدامه اختياري بالكامل.

---

## OpenStudier Go

OpenStudier Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenStudier، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenStudier.

1. شغّل الأمر `/connect` في TUI، واختر `OpenStudier Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع OpenStudier:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openstudier:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openstudier.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenStudier غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openstudier بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openstudier.
...
تخزين الرمز في مساحة تخزين مصادقة openstudier.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenStudier نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenStudier
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`openstudier.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `openstudier.json`:
...
لاستخدام اشتراك GitHub Copilot مع openstudier:
...
لاستخدام Google Vertex AI مع OpenStudier:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل openstudier.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openstudier، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) لتسجيل كل محادثة OpenStudier تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenStudier مدرجة كجلسة مستقلة.
...
يمكنك ضبط openstudier لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openstudier لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openstudier لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenStudier. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openstudier) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenStudier:
...
3. انسخ مفتاح API لاستخدامه في OpenStudier.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenStudier، يجب سحب معلومات النموذج محليا:
...
OpenStudier Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenStudier. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenStudier Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenStudier.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openstudier.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openstudier. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openstudier. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openstudier. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openstudier.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenStudier بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `openstudier auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openstudier وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openstudier.
