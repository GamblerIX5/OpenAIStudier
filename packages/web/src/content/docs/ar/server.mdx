---
title: الخادم
description: تفاعل مع خادم openstudier عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openstudier serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openstudier استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openstudier.local` |
...
عيّن `OPENSTUDIER_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openstudier`، أو عيّن `OPENSTUDIER_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openstudier serve` و `openstudier web`.
...
عند تشغيل `openstudier` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openstudier للتفاعل مع openstudier برمجيا.
:::

تتيح هذه البنية لـ openstudier دعم عدة عملاء وتمكّنك من التفاعل مع openstudier برمجيا.

يمكنك تشغيل `openstudier serve` لبدء خادم مستقل. إذا كانت واجهة openstudier في terminal (TUI)
قيد التشغيل، فسيبدأ `openstudier serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenStudier لـ [IDE](/docs/ide).
...
يعرض خادم openstudier واجهات API التالية.
