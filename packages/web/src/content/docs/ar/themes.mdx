...
مع OpenStudier يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenStudier سمتنا `openstudier`.
...
يأتي OpenStudier مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenStudier مظهر terminal لديهم
...
يدعم OpenStudier نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openstudier/themes/*.json` أو `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openstudier/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openstudier/themes/*.json`
