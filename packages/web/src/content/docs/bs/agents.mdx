...
Postoje dvije vrste agenata u OpenStudier: primarni agenti i podagenti.
...
OpenStudier dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenStudier dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenStudier dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `openstudier.json`:
...
- Globalno: `~/.config/openstudier/agents/`
- Po projektu: `.openstudier/agents/`
...
Ako temperatura nije navedena, OpenStudier koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenStudier konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenStudier konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenStudier Zen](/docs/zen), koristili biste `openstudier/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `openstudier models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/GamblerIX/OpenStudier).
:::
