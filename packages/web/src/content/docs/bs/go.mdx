...
OpenStudier Go je povoljna pretplata od **$10/mjesečno** koja vam daje pouzdan pristup popularnim otvorenim modelima kodiranja.

:::note
OpenStudier Go je trenutno u beta fazi.
:::

Go funkcioniše kao bilo koji drugi pružatelj usluga u OpenStudier-u. Pretplatite se na OpenStudier Go i
dobijete svoj API ključ. To je **potpuno opcionalno** i ne morate ga koristiti da biste
koristili OpenStudier.
...
:::tip
Testirali smo odabranu grupu modela i pružatelja usluga koji dobro rade sa OpenStudier-om.
:::
...
OpenStudier Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako to funkcioniše

OpenStudier Go funkcioniše kao bilo koji drugi pružatelj usluga u OpenStudier-u.

1. Prijavite se na **<a href={console}>OpenStudier Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite `/connect` komandu u TUI-ju, odaberite `OpenStudier Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih putem Go.

:::note
Samo jedan član po radnom prostoru se može pretplatiti na OpenStudier Go.
:::
...
OpenStudier Go uključuje sljedeća ograničenja:
...
OpenStudier Go je pretplatnički plan od **$10/mjesečno**. Ispod su cijene **po 1M tokena**.
...
[Model id](/docs/config/#models) u vašoj OpenStudier konfiguraciji
koristi format `openstudier-go/<model-id>`. Na primjer, za Kimi K2.5, biste
koristili `openstudier-go/kimi-k2.5` u vašoj konfiguraciji.
...
Kreirali smo OpenStudier Go da:
...
4. Nemamo **zaključavanja (lock-in)** omogućavajući vam da koristite bilo kojeg drugog pružatelja usluga sa OpenStudier-om također.
