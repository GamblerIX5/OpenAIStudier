---
title: LSP serveri
description: OpenStudier se integriše sa vašim LSP serverima.
---

OpenStudier se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

OpenStudier dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada openstudier otvori fajl, on:
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj openstudier konfiguraciji.
