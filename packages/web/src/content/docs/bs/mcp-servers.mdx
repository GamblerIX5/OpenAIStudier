...
Možete dodati vanjske alate u OpenStudier koristeći _Model Context Protocol_, ili MCP. OpenStudier podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenStudier Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openstudier`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenStudier automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenStudier će:
...
Ako server zahtijeva autentifikaciju, OpenStudier će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openstudier mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenStudier sigurno čuva tokene u `~/.local/share/openstudier/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenStudier, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenStudier konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenStudier sa vašim Sentry nalogom.
