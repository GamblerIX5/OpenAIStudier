...
ove ili konfigurirajte svoje putem openstudier config.
...
openstudier dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.openstudier/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `openstudier.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/openstudier/modes/`
- Projekat: `.openstudier/modes/`
...
Ako temperatura nije navedena, openstudier koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.openstudier/modes/` za specifične načine rada ili `~/.config/openstudier/modes/` za globalne načine:
