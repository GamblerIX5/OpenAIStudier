---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenStudier.
---

Dodaci vam omogućavaju da proširite OpenStudier spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenStudier.
...
- `.openstudier/plugins/` - Dodaci na nivou projekta
- `~/.config/openstudier/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openstudier/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openstudier/openstudier.json`)
2. Konfiguracija projekta (`openstudier.json`)
3. Globalni direktorij dodataka (`~/.config/openstudier/plugins/`)
4. Direktorij dodataka projekta (`.openstudier/plugins/`)
...
OpenStudier pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenStudier.
...
Spriječite openstudier da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openstudier:
...
Pomoćnik `tool` kreira prilagođeni alat koji openstudier može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openstudier zajedno sa ugrađenim alatima.
