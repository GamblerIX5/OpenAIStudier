---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenStudier koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenStudier konfiguraciji.
...
u `~/.local/share/openstudier/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenStudier
config.
...
OpenStudier Zen je lista modela koje je obezbedio OpenStudier tim koji su bili
testirano i potvrđeno da dobro radi sa OpenStudier. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenStudier Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openstudier i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u OpenStudier i potpuno je opcionalan za korištenje.

---

## OpenStudier Go

OpenStudier Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenStudier tim i koji su testirani i verificirani da dobro rade s OpenStudier-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenStudier Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s OpenStudier:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openstudier:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openstudier.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenStudier.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openstudier ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openstudier konfiguraciju.
...
da pohrani token u openstudier auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenStudier koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenStudier
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openstudier.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `openstudier.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openstudier:
...
Za korištenje Google Vertex AI s OpenStudier:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openstudier.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openstudiera, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) da automatski prijavite svaki OpenStudier razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenStudier razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openstudier za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openstudier za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openstudier za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenStudier. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openstudier) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenStudier:
...
3. Kopirajte API ključ za korištenje u OpenStudier.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenStudier, morate lokalno povući informacije o modelu:
...
OpenStudier Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenStudier tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenStudier Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenStudier Zen**.

```txt
   /connect
```

3. Unesite svoj OpenStudier API ključ.
...
Također možete dodati dodatne modele putem vaše openstudier konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openstudier konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju openstudier konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openstudier-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openstudier.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenStudier da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openstudier auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenStudier konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openstudier konfiguraciji.
