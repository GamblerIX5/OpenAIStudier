---
title: Server
description: Komunicirajte s openstudier serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openstudier serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openstudier klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openstudier.local` |
...
Postavite `OPENSTUDIER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openstudier`, ili postavite `OPENSTUDIER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openstudier serve` i za `openstudier web`.
...
Kada pokrenete `openstudier`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openstudier server za programsku interakciju sa openstudier-om.
:::

Ova arhitektura omogucava openstudier podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openstudier serve` da startate standalone server. Ako je openstudier TUI vec pokrenut, `openstudier serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenStudier [IDE](/docs/ide) pluginovi.
...
openstudier server izlaže sljedece API-je.
