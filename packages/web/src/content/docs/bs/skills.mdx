...
Agent skills omogucavaju OpenStudier da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenStudier pretrazuje ove lokacije:

- Konfiguracija projekta: `.openstudier/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openstudier/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenStudier ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openstudier/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openstudier/skills/git-release/SKILL.md` ovako:
...
OpenStudier navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openstudier.json`:
...
**Za ugradene agente** (u `openstudier.json`):
