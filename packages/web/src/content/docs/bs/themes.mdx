...
U OpenStudier mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenStudier koristi nasu `openstudier` temu.
...
OpenStudier dolazi sa vise ugradenih tema.
...
- Zele da OpenStudier odgovara izgledu njihovog terminala
...
OpenStudier podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openstudier/themes/*.json` ili `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openstudier/themes/*.json`
4. **Trenutni radni direktorij** - `./.openstudier/themes/*.json`
