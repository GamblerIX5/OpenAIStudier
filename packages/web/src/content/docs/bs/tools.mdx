...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenStudier dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenStudier.
...
:::note
Ovaj alat je dostupan samo kada je `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENSTUDIER_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenStudier provajdera ili kada je varijabla `OPENSTUDIER_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenStudier:
