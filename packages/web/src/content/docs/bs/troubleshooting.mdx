...
Da biste otklonili probleme s OpenStudier, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openstudier\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openstudier --log-level DEBUG`.

---

## Pohrana

OpenStudier pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openstudier`
...
OpenStudier Desktop pokreće lokalni OpenStudier server (`openstudier-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenStudier` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (ili `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
OpenStudier također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openstudier\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenStudier mogao ponovo izgraditi.

1. Potpuno zatvorite OpenStudier Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openstudier`
- **Linux**: obrišite `~/.cache/openstudier` (ili pokrenite `rm -rf ~/.cache/openstudier`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openstudier`

3. Ponovo pokrenite OpenStudier Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenStudier Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openstudier.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENSTUDIER_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENSTUDIER_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenStudier Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenStudier karakteristikama.

---

### Obavještenja se ne prikazuju

OpenStudier Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenStudier u postavkama vašeg OS-a, i
...
1. Zatvorite OpenStudier Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenStudier Desktop aplikacije):

- `openstudier.settings.dat` (URL zadanog servera za desktop)
- `openstudier.global.dat` i `openstudier.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenStudier:
...
Da saznate kojim modelima imate pristup, pokrenite `openstudier models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openstudier`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenStudier dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openstudier`

2. Ponovo pokrenite OpenStudier da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenStudier da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenStudier će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
