...
Der er to typer agenter i OpenStudier; primære agenter og subagenter.
...
OpenStudier leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
OpenStudier leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

OpenStudier leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `openstudier.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger OpenStudier modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale OpenStudier-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din OpenStudier-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenStudier Zen](/docs/zen), vil du bruge `openstudier/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `openstudier models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/GamblerIX/OpenStudier).
:::
