...
OpenStudier Go er et billigt **$10/måned** abonnement, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
OpenStudier Go er i øjeblikket i beta.
:::

Go fungerer ligesom enhver anden udbyder i OpenStudier. Du abonnerer på OpenStudier Go og får din API-nøgle. Det er **helt valgfrit**, og du behøver ikke at bruge det for at bruge OpenStudier.
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med OpenStudier.
:::
...
OpenStudier Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

OpenStudier Go fungerer ligesom enhver anden udbyder i OpenStudier.

1. Du logger ind på **<a href={console}>OpenStudier Zen</a>**, abonnerer på Go, og kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `OpenStudier Go`, og indsætter din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over modeller, der er tilgængelige gennem Go.

:::note
Kun ét medlem pr. workspace kan abonnere på OpenStudier Go.
:::
...
OpenStudier Go inkluderer følgende grænser:
...
OpenStudier Go er en **$10/måned** abonnementsplan. Nedenfor er priserne **pr. 1M tokens**.
...
[Model-id'et](/docs/config/#models) i din OpenStudier-konfiguration bruger formatet `openstudier-go/<model-id>`. For eksempel, for Kimi K2.5, ville du bruge `openstudier-go/kimi-k2.5` i din konfiguration.
...
Vi skabte OpenStudier Go for at:
...
4. Have **ingen lock-in** ved at tillade dig også at bruge enhver anden udbyder med OpenStudier.
