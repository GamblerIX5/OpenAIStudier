...
Du kan tilføje eksterne værktøjer til OpenStudier ved hjælp af _Model Context Protocol_ eller MCP. OpenStudier understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [OpenStudier Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/openstudier`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
OpenStudier håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil OpenStudier:
...
Hvis serveren kræver godkendelse, vil OpenStudier bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `openstudier mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil OpenStudier opbevare tokens sikkert i `~/.local/share/openstudier/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem OpenStudier-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde OpenStudier til din Sentry-konto.
