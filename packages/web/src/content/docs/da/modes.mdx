...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i openstudier-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i openstudier giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem openstudier-konfigurationen.
...
openstudier leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.openstudier/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `openstudier.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger openstudier modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale openstudier-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.openstudier/modes/` for projektspecifikke tilstande eller `~/.config/openstudier/modes/` for globale tilstande:
