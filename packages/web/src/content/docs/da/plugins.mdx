---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenStudier.
---

Plugins giver dig mulighed for at udvide OpenStudier ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenStudiers standardadfærd.
...
- `.openstudier/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openstudier/node_modules/`.
...
1. Global konfiguration (`~/.config/openstudier/openstudier.json`)
2. Projektkonfiguration (`openstudier.json`)
3. Global plugin-mappe (`~/.config/openstudier/plugins/`)
4. Projekt plugin bibliotek (`.openstudier/plugins/`)
...
OpenStudier kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openstudier SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openstudier.
...
:::note
Hvis du bruger OpenStudier desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openstudier i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openstudier:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openstudier kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openstudier sammen med indbyggede værktøjer.
