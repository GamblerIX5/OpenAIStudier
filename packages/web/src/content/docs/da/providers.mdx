---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenStudier bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenStudier-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openstudier/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenStudier-konfigurationen.
...
OpenStudier Zen er en liste over modeller leveret af OpenStudier-teamet, som er blevet testet og verificeret til at fungere godt med OpenStudier. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenStudier Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg openstudier og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i OpenStudier og er helt valgfrit at bruge.

---

## OpenStudier Go

OpenStudier Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenStudier-teamet, som er testet og verificeret til at fungere godt med OpenStudier.

1. Kør kommandoen `/connect` i TUI, vælg `OpenStudier Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med OpenStudier:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openstudier:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openstudier.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenStudier understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openstudier skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openstudier-konfigurationen.
...
gemme tokenet i openstudier auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenStudier bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenStudier
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`openstudier.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `openstudier.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenStudier:
...
Sådan bruger du Google Vertex AI med OpenStudier:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører openstudier.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openstudier, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) plugin for automatisk at logge hver OpenStudier-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenStudier-samtale opført som en separat session.
...
Du kan konfigurere openstudier til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openstudier til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openstudier til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenStudier. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openstudier) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenStudier:
...
3. Kopier API-nøglen til brug i OpenStudier.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenStudier, skal du hente modelinformationen lokalt:
...
OpenStudier Zen er en liste over testede og verificerede modeller leveret af OpenStudier-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenStudier Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenStudier API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openstudier-konfigurationen.
...
5. Du kan også tilpasse dem gennem openstudier-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenStudier:
...
Du kan også tilpasse modeller gennem openstudier-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openstudier. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openstudier.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenStudier forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `openstudier auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openstudier-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openstudier-konfigurationen.
