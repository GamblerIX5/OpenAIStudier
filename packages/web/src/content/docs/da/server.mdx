---
title: Server
description: Interager med openstudier-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openstudier serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openstudier-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openstudier.local` |
...
Indstil `OPENSTUDIER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openstudier`, eller indstil `OPENSTUDIER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openstudier serve` og `openstudier web`.
...
Når du kører `openstudier` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openstudier-serveren til at interagere med openstudier programmatisk.
:::

Denne arkitektur lader openstudier understøtte flere klienter og lader dig interagere med openstudier programmatisk.

Du kan køre `openstudier serve` for at starte en selvstændig server. Hvis du har
openstudier TUI kørende, vil `openstudier serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenStudier [IDE](/docs/ide) plugins.
...
OpenStudier-serveren viser følgende API'er.
