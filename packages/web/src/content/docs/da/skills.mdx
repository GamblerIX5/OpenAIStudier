...
Agentferdigheter lar OpenStudier oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenStudier søker etter disse stedene:

- Prosjektkonfigurasjon: `.openstudier/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openstudier/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenStudier opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openstudier/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openstudier/skills/git-release/SKILL.md` slik:
...
OpenStudier viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `openstudier.json`:
...
**For innebygde agenter** (i `openstudier.json`):
