...
Med OpenStudier kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenStudier vårt eget `openstudier`-tema.
...
OpenStudier kommer med flere innebygde temaer.
...
- Vil at OpenStudier skal matche terminalens utseende
...
OpenStudier støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openstudier/themes/*.json` eller `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openstudier/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openstudier/themes/*.json`
