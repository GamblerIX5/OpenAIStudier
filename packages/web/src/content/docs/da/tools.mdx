...
Her er alle de innebygde verktøyene som er tilgængelige i OpenStudier.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENSTUDIER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenStudier-leverandøren eller når miljøvariabelen `OPENSTUDIER_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
