...
For at fejlfinde problemer med OpenStudier, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openstudier\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openstudier --log-level DEBUG`.

---

## Lagring

openstudier gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openstudier`
...
OpenStudier Desktop kører en lokal OpenStudier-server (`openstudier-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenStudier`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (eller `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
OpenStudier kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openstudier\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenStudier kan genopbygge den.

1. Afslut OpenStudier Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openstudier`
- **Linux**: slet `~/.cache/openstudier` (eller kør `rm -rf ~/.cache/openstudier`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openstudier`

3. Start OpenStudier Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenStudier Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `openstudier.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENSTUDIER_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENSTUDIER_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver OpenStudier Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenStudier.

---

### Meddelelser vises ikke

OpenStudier Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenStudier i dine OS-indstillinger, og
...
1. Afslut OpenStudier Desktop.
2. Find og slet disse filer (de findes i OpenStudier Desktop-appens datamappe):

- `openstudier.settings.dat` (skrivebordsstandardserver URL)
- `openstudier.global.dat` og `openstudier.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenStudier:
...
For at finde ud af hvilke modeller du har adgang til, kør `openstudier models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openstudier`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openstudier installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openstudier`

2. Start openstudier på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openstudier til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openstudier vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
