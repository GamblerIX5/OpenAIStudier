...
Es gibt zwei Arten von Agenten in OpenStudier; Primäragenten und Subagenten.
...
OpenStudier verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `openstudier.json`-Konfigurationsdatei:
...
- Pro Projekt: `.openstudier/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet OpenStudier modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenStudier-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenStudier-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenStudier Zen](/docs/zen) verwenden, würden Sie `openstudier/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `openstudier models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/GamblerIX/OpenStudier).
:::
