...
OpenStudier Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte Open-Coding-Modelle bietet.

:::note
OpenStudier Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Anbieter in OpenStudier. Du abonnierst OpenStudier Go und erhältst deinen API-Schlüssel. Es ist **völlig optional** und du musst es nicht nutzen, um OpenStudier zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Anbietern getestet, die gut mit OpenStudier funktionieren.
:::
...
OpenStudier Go gibt dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

OpenStudier Go funktioniert wie jeder andere Anbieter in OpenStudier.

1. Du meldest dich bei **<a href={console}>OpenStudier Zen</a>** an, abonnierst Go und kopierst deinen API-Schlüssel.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenStudier Go` und fügst deinen API-Schlüssel ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenStudier Go abonnieren.
:::
...
OpenStudier Go beinhaltet die folgenden Limits:
...
OpenStudier Go ist ein Abonnementplan für **10 $/Monat**. Unten stehen die Preise **pro 1 Mio. Token**.
...
Die [Modell-ID](/docs/config/#models) in deiner OpenStudier-Konfiguration verwendet das Format `openstudier-go/<model-id>`. Zum Beispiel würdest du für Kimi K2.5 `openstudier-go/kimi-k2.5` in deiner Konfiguration verwenden.
...
Wir haben OpenStudier Go erstellt, um:
...
4. **Keinen Lock-in** zu haben, indem wir dir ermöglichen, jeden anderen Anbieter ebenfalls mit OpenStudier zu nutzen.
