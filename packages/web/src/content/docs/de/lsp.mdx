---
title: LSP Server
description: OpenStudier lässt sich in Ihren LSP-Server integrieren.
---

OpenStudier lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

OpenStudier verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn OpenStudier eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer OpenStudier-Konfiguration anpassen.
...
```json title="openstudier.json" {5-7}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="openstudier.json" {5-9}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "typescript": {
