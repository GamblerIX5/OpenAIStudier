---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenStudier zu erweitern.
---

Mit Plugins können Sie OpenStudier erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenStudier zu ändern.
...
- `.openstudier/plugins/` – Plugins auf Projektebene
- `~/.config/openstudier/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openstudier/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openstudier/openstudier.json`)
2. Projektkonfiguration (`openstudier.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openstudier/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openstudier/plugins/`)
...
OpenStudier führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenStudier-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenStudier erweitern können.
...
:::note
Wenn Sie die OpenStudier-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenStudier `.env`-Dateien liest:
...
Plugins können OpenStudier auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenStudier aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenStudier zur Verfügung.
