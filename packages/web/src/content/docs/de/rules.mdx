---
title: Regeln
description: Lege eigene Anweisungen fuer openstudier fest.
---

Du kannst openstudier ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openstudier den Befehl `/init` aus.
...
So kann openstudier sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openstudier sie zu erweitern.
...
openstudier kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openstudier/AGENTS.md` speichern.
Diese gelten in allen openstudier-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenStudier dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openstudier/AGENTS.md` existiert)
...
Beim Start sucht openstudier in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openstudier/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openstudier.json` oder in `~/.config/openstudier/openstudier.json` angeben.
...
openstudier parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openstudier.json

Empfohlen ist das `instructions`-Feld in der `openstudier.json`:
...
Du kannst openstudier explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openstudier.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
