---
title: Server
description: Nutze den openstudier-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openstudier serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openstudier-Client nutzen kann.
...
Setze `OPENSTUDIER_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openstudier`, kann aber mit `OPENSTUDIER_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openstudier serve` und `openstudier web`.
...
Wenn du `openstudier` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openstudier-Server, um openstudier programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openstudier serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openstudier serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenStudier-[IDE](/docs/ide)-Plugins.
...
Der openstudier-Server stellt folgende APIs bereit.
