...
Agent Skills erlauben OpenStudier, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenStudier sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenStudier vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openstudier/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openstudier/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenStudier listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openstudier.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openstudier.json`):
