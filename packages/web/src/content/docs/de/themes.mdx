...
Mit OpenStudier kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenStudier das `openstudier`-Theme.
...
OpenStudier liefert mehrere eingebaute Themes mit.
...
- OpenStudier optisch ans Terminal anpassen wollen
...
OpenStudier unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openstudier/themes/*.json` oder `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openstudier/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openstudier/themes/*.json`
