...
OpenStudier bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in OpenStudier verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (oder `OPENSTUDIER_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenStudier-Provider genutzt wird oder die Umgebungsvariable `OPENSTUDIER_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von OpenStudier:
