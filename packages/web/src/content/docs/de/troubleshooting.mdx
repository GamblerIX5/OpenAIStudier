...
Wenn OpenStudier Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openstudier\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openstudier --log-level DEBUG`.

---

## Speicher

openstudier speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openstudier`
...
OpenStudier Desktop startet im Hintergrund einen lokalen OpenStudier-Server (`openstudier-cli`-Sidecar).
...
- Nur macOS: `OpenStudier`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (oder `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
OpenStudier kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openstudier\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenStudier ihn neu aufbauen kann.

1. Beende OpenStudier Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openstudier`
- **Linux**: loesche `~/.cache/openstudier` (oder fuehre aus: `rm -rf ~/.cache/openstudier`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openstudier`

3. Starte OpenStudier Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenStudier Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `openstudier.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENSTUDIER_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENSTUDIER_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt OpenStudier Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenStudier-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenStudier Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenStudier in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenStudier Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenStudier Desktop):

- `openstudier.settings.dat` (desktop default server URL)
- `openstudier.global.dat` und `openstudier.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenStudier hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openstudier models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openstudier`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openstudier installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openstudier`

2. Starte openstudier neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openstudier, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openstudier erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
