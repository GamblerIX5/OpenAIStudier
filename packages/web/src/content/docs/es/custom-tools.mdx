...
description: Cree herramientas que LLM pueda llamar en openstudier.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openstudier como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openstudier/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openstudier/tools/`.
...
```ts title=".openstudier/tools/database.ts" {1}
...
```ts title=".openstudier/tools/math.ts"
import { tool } from "@openstudier/plugin"
...
```ts title=".openstudier/tools/bash.ts"
import { tool } from "@openstudier/plugin"
...
```ts title=".openstudier/tools/project.ts" {8}
import { tool } from "@openstudier/plugin"
...
```python title=".openstudier/tools/add.py"
import sys
...
```ts title=".openstudier/tools/python-add.ts" {10}
import { tool } from "@openstudier/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openstudier/tools/add.py")
