---
title: Complementos
description: Escriba sus propios complementos para extender OpenStudier.
---

Los complementos le permiten extender OpenStudier al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenStudier.
...
- `.openstudier/plugins/` - Complementos a nivel de proyecto
- `~/.config/openstudier/plugins/` - Complementos globales
...
```json title="openstudier.json"
{
  ...
  "plugin": ["openstudier-helicone-session", "openstudier-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openstudier/node_modules/`.
...
1. Configuración global (`~/.config/openstudier/openstudier.json`)
2. Configuración del proyecto (`openstudier.json`)
3. Directorio global de complementos (`~/.config/openstudier/plugins/`)
4. Directorio de complementos del proyecto (`.openstudier/plugins/`)
...
```json title=".openstudier/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenStudier ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openstudier/plugins/my-plugin.ts"
...
```js title=".openstudier/plugins/example.js"
...
- `client`: Un cliente SDK openstudier para interactuar con la IA.
...
import type { Plugin } from "@openstudier/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openstudier.
...
```js title=".openstudier/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openstudier"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenStudier, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openstudier lea archivos `.env`:

```javascript title=".openstudier/plugins/env-protection.js"
...
```javascript title=".openstudier/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openstudier:

```ts title=".openstudier/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openstudier/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openstudier puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openstudier junto con las herramientas integradas.
...
```ts title=".openstudier/plugins/compaction.ts"
import type { Plugin } from "@openstudier/plugin"
...
```ts title=".openstudier/plugins/custom-compaction.ts"
import type { Plugin } from "@openstudier/plugin"
