---
title: Servidor
description: Interactuar con el servidor openstudier a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openstudier serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openstudier puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openstudier.local` |
...
Configure `OPENSTUDIER_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openstudier`, o configure `OPENSTUDIER_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openstudier serve` como a `openstudier web`.
...
Cuando ejecuta `openstudier`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openstudier para interactuar con openstudier mediante programación.
:::

Esta arquitectura permite que openstudier admita múltiples clientes y le permite interactuar con openstudier mediante programación.

Puede ejecutar `openstudier serve` para iniciar un servidor independiente. Si tienes el
openstudier TUI ejecutándose, `openstudier serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenStudier [IDE](/docs/ide).
...
El servidor openstudier expone las siguientes API.
