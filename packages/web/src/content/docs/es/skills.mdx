...
Las habilidades del agente permiten a OpenStudier descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenStudier busca estas ubicaciones:

- Configuración del proyecto: `.openstudier/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openstudier/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenStudier sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openstudier/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openstudier/skills/git-release/SKILL.md` así:
...
OpenStudier enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openstudier.json`:
...
**Para agentes integrados** (en `openstudier.json`):
