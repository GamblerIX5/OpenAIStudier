...
Las herramientas permiten que LLM realice acciones en su código base. OpenStudier viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenStudier.
...
:::note
Esta herramienta solo está disponible cuando `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (o `OPENSTUDIER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenStudier o cuando la variable de entorno `OPENSTUDIER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenStudier:

```bash
OPENSTUDIER_ENABLE_EXA=1 openstudier
```
