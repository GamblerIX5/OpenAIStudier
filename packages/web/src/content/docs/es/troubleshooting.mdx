...
Para depurar problemas con OpenStudier, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/openstudier/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openstudier\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openstudier --log-level DEBUG`.

---

## Almacenamiento

openstudier almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openstudier`
...
OpenStudier Desktop ejecuta un servidor OpenStudier local (el sidecar `openstudier-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenStudier` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (o `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
OpenStudier también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openstudier\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenStudier pueda reconstruirlo.

1. Salga de OpenStudier Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openstudier`
- **Linux**: eliminar `~/.cache/openstudier` (o ejecutar `rm -rf ~/.cache/openstudier`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openstudier`

3. Reinicie el escritorio OpenStudier.

---

### Solucionar problemas de conexión del servidor

OpenStudier Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `openstudier.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENSTUDIER_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENSTUDIER_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio OpenStudier requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenStudier.

---

### Notificaciones que no se muestran

OpenStudier Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenStudier en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenStudier.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenStudier):

- `openstudier.settings.dat` (URL del servidor predeterminado de escritorio)
- `openstudier.global.dat` y `openstudier.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenStudier:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/GamblerIX/OpenStudier/issues**](https://github.com/GamblerIX/OpenStudier/issues)
...
3. Asegúrese de tener la última versión con `openstudier upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openstudier models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/openstudier
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openstudier`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openstudier instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openstudier`

2. Reinicie openstudier para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openstudier a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openstudier detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
