---
title: Zen
description: Lista seleccionada de modelos proporcionada por OpenStudier.
---
...
OpenStudier Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenStudier.

:::note
OpenStudier Zen se encuentra actualmente en versión beta.
:::

Zen funciona como cualquier otro proveedor en OpenStudier. Inicias sesion en OpenStudier Zen y obtienes
tu API key. Es **completamente opcional** y no necesitas usarlo para usar OpenStudier.
...
:::tip
Probamos un grupo selecto de modelos y proveedores que funcionan bien con OpenStudier.
:::
...
OpenStudier Zen es una puerta de enlace de IA que le brinda acceso a estos modelos.

---

## Cómo funciona

OpenStudier Zen funciona como cualquier otro proveedor en OpenStudier.

1. Inicias sesión en **<a href={console}>OpenStudier Zen</a>**, agregas tu facturación
   detalles y copie su clave API.
2. Ejecuta el comando `/connect` en TUI, selecciona OpenStudier Zen y pega tu clave API.
...
El [model ID](/docs/config/#models) en tu configuracion de OpenStudier
usa el formato `openstudier/<model-id>`. Por ejemplo, para GPT 5.2 Codex, debes
usar `openstudier/gpt-5.2-codex` en tu configuracion.
...
- MiniMax M2.5 Free está disponible en OpenStudier por tiempo limitado. El equipo está aprovechando este tiempo para recopilar comentarios y mejorar el modelo.
- Big Pickle es un modelo sigiloso gratuito en OpenStudier por tiempo limitado. El equipo está aprovechando este tiempo para recopilar comentarios y mejorar el modelo.
...
4. **No tener ningún bloqueo** al permitirle usarlo con cualquier otro agente de codificación. Y siempre le permitirá utilizar cualquier otro proveedor con OpenStudier también.
