...
Il existe deux types d'agents dans OpenStudier ; agents primaires et sous-agents.
...
OpenStudier est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenStudier est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenStudier est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `openstudier.json` :
...
- Global : `~/.config/openstudier/agents/`
- Par projet : `.openstudier/agents/`

```markdown title="~/.config/openstudier/agents/review.md"
---
...
Si aucune température n'est spécifiée, OpenStudier utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenStudier et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenStudier utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenStudier Zen](/docs/zen), vous utiliserez `openstudier/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="openstudier.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="openstudier.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="openstudier.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `openstudier models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/GamblerIX/OpenStudier).
:::
...
```markdown title="~/.config/openstudier/agents/docs-writer.md"
---
...
```markdown title="~/.config/openstudier/agents/security-auditor.md"
---
