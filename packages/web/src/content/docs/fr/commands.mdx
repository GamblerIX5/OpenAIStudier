...
Créez `.openstudier/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration OpenStudier ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OpenStudier [config](/docs/config) :
...
- Par projet : `.openstudier/commands/`
...
openstudier comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
