---
title: Configuration
description: Utilisation de la configuration JSON OpenStudier.
---

Vous pouvez configurer OpenStudier à l'aide d'un fichier de configuration JSON.

---

## Format

OpenStudier prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="openstudier.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/openstudier`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/openstudier/openstudier.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENSTUDIER_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`openstudier.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.openstudier`** - agents, commandes, plugins
6. **Configuration en ligne** (`OPENSTUDIER_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.openstudier` et `~/.config/openstudier` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/openstudier`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/openstudier"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale OpenStudier dans `~/.config/openstudier/openstudier.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/openstudier/tui.json`.
...
Ajoutez `openstudier.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque OpenStudier démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `OPENSTUDIER_CONFIG`.

```bash
export OPENSTUDIER_CONFIG=/path/to/my/custom-config.json
openstudier run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENSTUDIER_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.openstudier`, et devrait suivre la même structure.

```bash
export OPENSTUDIER_CONFIG_DIR=/path/to/my/config-directory
openstudier run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.openstudier`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `OPENSTUDIER_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `openstudier.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `openstudier serve` et `openstudier web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur OpenStudier.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `openstudier.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration OpenStudier via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, OpenStudier essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/openstudier/agents/` ou `.openstudier/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, OpenStudier reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`openstudier run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/openstudier/commands/` ou `.openstudier/commands/`. [En savoir plus ici](/docs/commands).
...
OpenStudier téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, openstudier **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent OpenStudier avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.openstudier/plugins/` ou `~/.config/openstudier/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="openstudier.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["openstudier-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre OpenStudier à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
