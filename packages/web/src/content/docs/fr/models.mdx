...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez OpenStudier.
...
Voici plusieurs modèles qui fonctionnent bien avec OpenStudier, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration OpenStudier.

```json title="openstudier.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [OpenStudier Zen](/docs/zen), vous utiliserez `openstudier/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="openstudier.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="openstudier.jsonc" {6-21}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openstudier": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. OpenStudier est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

OpenStudier est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="openstudier.jsonc" {7-18}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque OpenStudier démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration OpenStudier.
