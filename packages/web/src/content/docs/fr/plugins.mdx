---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenStudier.
---

Les plugins vous permettent d'étendre OpenStudier en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenStudier.
...
- `.openstudier/plugins/` - Plugins au niveau du projet
- `~/.config/openstudier/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openstudier/node_modules/`.
...
1. Configuration globale (`~/.config/openstudier/openstudier.json`)
2. Configuration du projet (`openstudier.json`)
3. Répertoire global des plugins (`~/.config/openstudier/plugins/`)
4. Répertoire des plugins du projet (`.openstudier/plugins/`)
...
OpenStudier exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openstudier pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openstudier.
...
:::note
Si vous utilisez l'application de bureau OpenStudier, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openstudier de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openstudier :
...
L'assistant `tool` crée un outil personnalisé que openstudier peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openstudier aux côtés des outils intégrés.
