---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenStudier utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenStudier.
...
en `~/.local/share/openstudier/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenStudier
configuration.
...
OpenStudier Zen est une liste de modèles fournis par l'équipe OpenStudier qui ont été
testé et vérifié pour fonctionner correctement avec OpenStudier. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenStudier Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez openstudier et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenStudier et son utilisation est totalement facultative.

---

## OpenStudier Go

OpenStudier Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe OpenStudier qui ont été
testé et vérifié pour fonctionner correctement avec OpenStudier.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenStudier Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec OpenStudier :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de openstudier :
...
Pour une configuration spécifique au projet ou persistante, utilisez `openstudier.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenStudier n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que openstudier fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration openstudier.
...
pour stocker le jeton dans le stockage d'authentification openstudier.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenStudier utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenStudier
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `openstudier.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuration

Personnalisez via `openstudier.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec openstudier :
...
Pour utiliser Google Vertex AI avec OpenStudier :
...
Définissez-les lors de l'exécution de openstudier.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via openstudier, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) pour enregistrer automatiquement chaque conversation OpenStudier en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenStudier répertoriée comme une session distincte.
...
Vous pouvez configurer openstudier pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer openstudier pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer openstudier pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenStudier. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/openstudier) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenStudier :
...
3. Copiez la clé API à utiliser dans OpenStudier.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenStudier, vous devez extraire les informations du modèle localement :
...
OpenStudier Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenStudier. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenStudier Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenStudier API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration openstudier.
...
5. Vous pouvez également les personnaliser via votre configuration openstudier. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration openstudier. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec openstudier. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `openstudier.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à OpenStudier de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `openstudier auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration openstudier et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration openstudier.
