---
title: SDK
description: Client JS de type sécurisé pour le serveur openstudier.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK openstudier JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler openstudier par programme.
...
Créez une instance de openstudier :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `openstudier.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${openstudier.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de openstudier, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@openstudier/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*openstudier" },
})
