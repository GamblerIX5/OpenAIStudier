---
title: Serveur
description: Interagissez avec le serveur openstudier via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openstudier serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openstudier peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openstudier.local` |
...
Définissez `OPENSTUDIER_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openstudier`, ou définissez `OPENSTUDIER_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openstudier serve` et à `openstudier web`.
...
Lorsque vous exécutez `openstudier`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openstudier pour interagir avec openstudier par programmation.
:::

Cette architecture permet à openstudier de prendre en charge plusieurs clients et vous permet d'interagir avec openstudier par programme.

Vous pouvez exécuter `openstudier serve` pour démarrer un serveur autonome. Si vous avez le
openstudier TUI en cours d'exécution, `openstudier serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenStudier [IDE](/docs/ide).
...
Le serveur openstudier expose les API suivantes.
