...
Avec OpenStudier, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenStudier utilise notre propre thème `openstudier`.
...
OpenStudier est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenStudier corresponde à l'apparence de leur terminal
...
OpenStudier prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openstudier/themes/*.json` ou `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openstudier/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openstudier/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openstudier/themes
vim ~/.config/openstudier/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openstudier/themes
vim .openstudier/themes/my-theme.json
```
