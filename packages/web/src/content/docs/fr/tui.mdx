---
title: TUI
description: Utilisation de l’interface utilisateur du terminal OpenStudier.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de OpenStudier démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le OpenStudier TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à OpenStudier. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez OpenStudier. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `openstudier.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `OPENSTUDIER_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
