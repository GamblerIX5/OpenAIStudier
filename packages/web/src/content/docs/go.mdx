...
OpenStudier Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
OpenStudier Go is currently in beta.
:::

Go works like any other provider in OpenStudier. You subscribe to OpenStudier Go and
...
OpenStudier Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

OpenStudier Go works like any other provider in OpenStudier.

1. You sign in to **<a href={console}>OpenStudier Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `OpenStudier Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to OpenStudier Go.
:::
...
OpenStudier Go includes the following limits:
...
uses the format `openstudier-go/<model-id>`. For example, for Kimi K2.5, you would
use `openstudier-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with OpenStudier as well.
