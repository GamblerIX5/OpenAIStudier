...
In OpenStudier esistono due tipi di agenti: agenti primari e subagenti.
...
OpenStudier include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenStudier include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenStudier include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `openstudier.json`:
...
- Globale: `~/.config/openstudier/agents/`
- Per progetto: `.openstudier/agents/`
...
Se non specifichi una temperature, OpenStudier usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenStudier sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenStudier usa il formato `provider/model-id`. Per esempio, se stai usando [OpenStudier Zen](/docs/zen), useresti `openstudier/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `openstudier models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/GamblerIX/OpenStudier).
:::
