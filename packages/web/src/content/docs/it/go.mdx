...
OpenStudier Go è un abbonamento a basso costo di **$10/mese** che ti offre un accesso affidabile ai modelli di coding open source più popolari.

:::note
OpenStudier Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in OpenStudier. Ti abboni a OpenStudier Go e ottieni la tua chiave API. È **completamente opzionale** e non è necessario utilizzarlo per usare OpenStudier.
...
OpenStudier Go ti dà accesso a questi modelli per **$10/mese**.

---

## Come funziona

OpenStudier Go funziona come qualsiasi altro provider in OpenStudier.

1. Accedi a **<a href={console}>OpenStudier Zen</a>**, abbonati a Go e copia la tua chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona `OpenStudier Go` e incolla la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OpenStudier Go.
:::
...
OpenStudier Go include i seguenti limiti:
...
OpenStudier Go è un piano di abbonamento da **$10/mese**. Di seguito sono riportati i prezzi **per 1M di token**.
...
Il [model id](/docs/config/#models) nella tua configurazione OpenStudier utilizza il formato `openstudier-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti `openstudier-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato OpenStudier Go per:
...
4. Non avere **alcun lock-in** permettendoti di utilizzare qualsiasi altro provider con OpenStudier.
