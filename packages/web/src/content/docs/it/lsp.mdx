---
title: Server LSP
description: OpenStudier si integra con i tuoi server LSP.
---

OpenStudier si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

OpenStudier include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando openstudier apre un file:
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di openstudier.
