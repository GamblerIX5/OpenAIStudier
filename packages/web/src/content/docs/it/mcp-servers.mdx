...
Puoi aggiungere strumenti esterni a OpenStudier usando il _Model Context Protocol_, o MCP. OpenStudier supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenStudier Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/openstudier`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenStudier gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenStudier:
...
Se il server richiede autenticazione, OpenStudier ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `openstudier mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenStudier memorizzera' i token in modo sicuro in `~/.local/share/openstudier/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenStudier insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenStudier come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenStudier al tuo account Sentry.
