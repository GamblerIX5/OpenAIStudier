...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di openstudier. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in openstudier ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di openstudier.
...
openstudier include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.openstudier/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `openstudier.json`:
...
- Globale: `~/.config/openstudier/modes/`
- Progetto: `.openstudier/modes/`
...
Se non viene specificata alcuna temperatura, openstudier usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di openstudier sia per quella specifica del progetto.
...
Crea file di modalita in `.openstudier/modes/` per modalita specifiche del progetto oppure in `~/.config/openstudier/modes/` per modalita globali:
