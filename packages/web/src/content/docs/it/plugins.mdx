---
title: Plugin
description: Scrivi plugin per estendere OpenStudier.
---

I plugin ti permettono di estendere OpenStudier agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenStudier.
...
- `.openstudier/plugins/` - plugin a livello progetto
- `~/.config/openstudier/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openstudier/node_modules/`.
...
1. Config globale (`~/.config/openstudier/openstudier.json`)
2. Config di progetto (`openstudier.json`)
3. Directory plugin globale (`~/.config/openstudier/plugins/`)
4. Directory plugin di progetto (`.openstudier/plugins/`)
...
OpenStudier esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openstudier per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openstudier.
...
:::note
Se usi l'app desktop di OpenStudier, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openstudier di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openstudier:
...
L'helper `tool` crea uno strumento personalizzato che openstudier può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openstudier insieme agli strumenti integrati.
