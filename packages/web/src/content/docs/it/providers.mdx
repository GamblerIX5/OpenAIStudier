---
title: Provider
description: Usa qualsiasi provider LLM in OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenStudier usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenStudier.
...
OpenStudier Zen e una lista di modelli forniti dal team OpenStudier, testati e
verificati per funzionare bene con OpenStudier. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenStudier Zen.
:::

1. Esegui `/connect` nella TUI, seleziona openstudier e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in OpenStudier ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenStudier:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openstudier:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `openstudier.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenStudier non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openstudier funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openstudier.
...
salvare il token nello storage di auth di openstudier.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OpenStudier usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenStudier
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `openstudier.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configurazione

Personalizza tramite `openstudier.json`:
...
Per usare il tuo abbonamento GitHub Copilot con openstudier:
...
Per usare Google Vertex AI con OpenStudier:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openstudier.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openstudier, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) per loggare automaticamente ogni conversazione di OpenStudier come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenStudier elencata come sessione separata.
...
Puoi configurare openstudier per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openstudier per usare modelli locali tramite LM Studio.
...
Puoi configurare openstudier per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenStudier. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openstudier) per dettagli.
:::
...
Per usare Ollama Cloud con OpenStudier:
...
3. Copia la chiave API da usare in OpenStudier.
...
6. **Importante**: Prima di usare modelli cloud in OpenStudier, devi scaricare le informazioni del modello localmente:
...
OpenStudier Zen è una lista di modelli testati e verificati forniti dal team OpenStudier. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenStudier Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenStudier.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openstudier.
...
5. Puoi anche personalizzarli tramite la configurazione di openstudier. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openstudier. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openstudier. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `openstudier.json` nella directory del progetto:
...
I campi `limit` permettono a OpenStudier di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `openstudier auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openstudier e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openstudier.
