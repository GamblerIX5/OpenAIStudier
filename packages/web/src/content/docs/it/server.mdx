---
title: Server
description: Interagisci con il server openstudier via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openstudier serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openstudier.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openstudier.local` |
...
Imposta `OPENSTUDIER_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openstudier`, oppure imposta `OPENSTUDIER_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openstudier serve` sia per `openstudier web`.
...
Quando esegui `openstudier` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openstudier per interagire con openstudier in modo programmatico.
:::

Questa architettura permette a openstudier di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openstudier serve` per avviare un server standalone. Se la TUI di openstudier e gia in esecuzione, `openstudier serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenStudier.
...
Il server openstudier espone le seguenti API.
