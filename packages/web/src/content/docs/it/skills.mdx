...
Le skill degli agenti permettono a OpenStudier di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenStudier cerca in queste posizioni:

- Config di progetto: `.openstudier/skills/<name>/SKILL.md`
- Config globale: `~/.config/openstudier/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenStudier risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openstudier/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openstudier/skills/git-release/SKILL.md` cosi':
...
OpenStudier elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openstudier.json`:
...
**Per agenti integrati** (in `openstudier.json`):
