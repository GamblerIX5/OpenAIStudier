---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenStudier.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenStudier avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenStudier, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenStudier. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenStudier. _Alias_: `/quit`, `/q`
...
Questo è separato da `openstudier.json`, che configura il comportamento di server/runtime.
...
Usa `OPENSTUDIER_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
