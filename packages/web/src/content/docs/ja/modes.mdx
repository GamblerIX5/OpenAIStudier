...
:::caution
モードは、OpenStudier 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、OpenStudier 設定を通じて独自の設定を行います。
...
OpenStudier には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.openstudier/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`openstudier.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/openstudier/modes/`
- プロジェクト: `.openstudier/modes/`
...
温度が指定されていない場合、OpenStudier はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.openstudier/modes/` に、グローバルモードの場合は `~/.config/openstudier/modes/` に作成します。
