---
title: プラグイン
description: OpenStudier を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで OpenStudier を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、OpenStudier のデフォルトの動作を変更したりできます。
...
- `.openstudier/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/openstudier/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/openstudier/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/openstudier/openstudier.json`)
2. プロジェクト設定 (`openstudier.json`)
3. グローバルプラグインディレクトリ (`~/.config/openstudier/plugins/`)
4. プロジェクトプラグインディレクトリ (`.openstudier/plugins/`)
...
OpenStudier は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための OpenStudier SDK クライアント。
...
ここでは、OpenStudier を拡張するために使用できるプラグインの例をいくつか示します。
...
OpenStudier が `.env` ファイルを読み取らないようにします。
...
プラグインは OpenStudier にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、OpenStudier が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して OpenStudier で使用できます。
