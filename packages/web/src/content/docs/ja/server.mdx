---
title: サーバー
description: HTTP 経由で OpenStudier サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openstudier serve` コマンドは、OpenStudier クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `openstudier.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENSTUDIER_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `openstudier` になるか、`OPENSTUDIER_SERVER_USERNAME` を設定してオーバーライドします。これは、`openstudier serve` と `openstudier web` の両方に当てはまります。
...
`openstudier` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
OpenStudier サーバーを使用して、プログラムで OpenStudier と対話します。
:::

`openstudier serve` を実行してスタンドアロンサーバーを起動できます。すでに OpenStudier TUI を実行している場合でも、`openstudier serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、OpenStudier [IDE](/docs/ide) プラグインによって使用されます。
...
OpenStudier サーバーは次の API を公開します。
