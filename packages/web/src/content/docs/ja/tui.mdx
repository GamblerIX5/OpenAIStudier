---
title: TUI
description: OpenStudier ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

OpenStudier を実行すると、現在のディレクトリの TUI が開始されます。
...
OpenStudier TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
OpenStudier にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
OpenStudier を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `openstudier.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`OPENSTUDIER_TUI_CONFIG` を使用します。
