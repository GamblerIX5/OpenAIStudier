---
title: OpenStudier Zen
description: OpenStudier によって提供されるモデルの厳選されたリスト。
---
...
OpenStudier Zen は、OpenStudier チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
OpenStudier Zen は現在ベータ版です。
:::
Zen は OpenStudier の他のプロバイダーと同様に機能します。 OpenStudier Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、OpenStudier を使用するために使用する必要はありません。
...
:::tip
OpenStudier で適切に動作する、厳選されたモデルとプロバイダーのグループをテストしました。
:::
...
OpenStudier Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

OpenStudier Zen は、OpenStudier の他のプロバイダーと同様に機能します。

1. **<a href={console}>OpenStudier Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、OpenStudier Zen を選択して API キーを貼り付けます。
...
OpenStudier 設定の [model id](/docs/config/#models)
`openstudier/<model-id>` 形式を使用します。たとえば、GPT 5.2 Codex の場合は、
設定で `openstudier/gpt-5.2-codex` を使用してください。
...
- MiniMax M2.5 Free は期間限定で OpenStudier で利用可能です。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で OpenStudier で無料で利用できるステルスモデルです。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
...
私たちは次の目的で OpenStudier Zen を作成しました。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に OpenStudier で他のプロバイダーも使用できるようにします。
