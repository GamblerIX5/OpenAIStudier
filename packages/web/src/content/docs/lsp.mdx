---
title: LSP Servers
description: OpenStudier integrates with your LSP servers.
---

OpenStudier integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

OpenStudier comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When openstudier opens a file, it:
...
You can customize LSP servers through the `lsp` section in your openstudier config.
