...
You can add external tools to OpenStudier using the _Model Context Protocol_, or MCP. OpenStudier supports both local and remote servers.
...
You can define MCP servers in your [OpenStudier Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openstudier` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenStudier automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenStudier will:
...
If the server requires authentication, OpenStudier will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openstudier mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenStudier will store the tokens securely in `~/.local/share/openstudier/mcp-auth.json`.
...
Your MCPs are available as tools in OpenStudier, alongside built-in tools. So you can manage them through the OpenStudier config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenStudier to your Sentry account.
