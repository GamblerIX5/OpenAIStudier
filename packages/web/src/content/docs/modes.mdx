...
:::caution
Modes are now configured through the `agent` option in the openstudier config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in openstudier allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the openstudier config.
...
openstudier comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.openstudier/plans/*.md` to detail the plan itself
...
Configure modes in your `openstudier.json` config file:
...
both the global openstudier config and the project specific config.
...
Create mode files in `.openstudier/modes/` for project-specific modes or `~/.config/openstudier/modes/` for global modes:
