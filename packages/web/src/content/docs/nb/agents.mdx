...
Det er to typer agenter i OpenStudier; primære agenter og underagenter.
...
OpenStudier kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenStudier kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenStudier kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openstudier.json`:
...
- Globalt: `~/.config/openstudier/agents/`
- Per prosjekt: `.openstudier/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenStudier modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenStudier-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenStudier-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenStudier Zen](/docs/zen), vil du bruke `openstudier/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openstudier models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/GamblerIX/OpenStudier).
:::
