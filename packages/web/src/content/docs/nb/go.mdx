...
OpenStudier Go er et lavkostnadsabonnement til **$10/måned** som gir deg pålitelig tilgang til populære åpne kodemodeller.

:::note
OpenStudier Go er for tiden i beta.
:::

Go fungerer som enhver annen leverandør i OpenStudier. Du abonnerer på OpenStudier Go og
...
:::tip
Vi testet en utvalgt gruppe modeller og leverandører som fungerer bra med OpenStudier.
:::
...
OpenStudier Go gir deg tilgang til disse modellene for **$10/måned**.

---

## Hvordan det fungerer

OpenStudier Go fungerer som enhver annen leverandør i OpenStudier.

1. Du logger deg inn på **<a href={console}>OpenStudier Zen</a>**, abonnerer på Go, og
   kopierer API-nøkkelen din.
2. Du kjører kommandoen `/connect` i TUI-en, velger `OpenStudier Go`, og limer inn
   API-nøkkelen din.
3. Kjør `/models` i TUI-en for å se listen over modeller tilgjengelig gjennom Go.

:::note
Bare ett medlem per arbeidsområde kan abonnere på OpenStudier Go.
:::
...
OpenStudier Go inkluderer følgende grenser:
...
OpenStudier Go er et **$10/måned** abonnementsplan. Nedenfor er prisene **per 1M tokens**.
...
[Modell-ID-en](/docs/config/#models) i din OpenStudier-konfigurasjon
bruker formatet `openstudier-go/<model-id>`. For eksempel, for Kimi K2.5, ville du
bruke `openstudier-go/kimi-k2.5` i konfigurasjonen din.
...
Vi opprettet OpenStudier Go for å:
...
4. Ha **ingen innlåsing** ved å tillate deg å bruke hvilken som helst annen leverandør med OpenStudier også.
