---
title: LSP-servere
description: openstudier integreres med LSP-serverne dine.
---

OpenStudier integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

OpenStudier kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når openstudier åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i OpenStudier-konfigurasjonen.
