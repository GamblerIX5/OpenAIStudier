...
Du kan legge til eksterne verktøy i OpenStudier ved å bruke _Model Context Protocol_ eller MCP. OpenStudier støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenStudier Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openstudier.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openstudier`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="openstudier.jsonc" {15}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenStudier håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenStudier:
...
Hvis serveren krever autentisering, vil OpenStudier be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openstudier mcp auth <server-name>`.
...
```json title="openstudier.json" {7-11}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenStudier lagre tokens sikkert i `~/.local/share/openstudier/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i OpenStudier, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenStudier-konfigurasjonen som alle andre verktøy.
...
```json title="openstudier.json" {14}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="openstudier.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="openstudier.json" {4-8}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "sentry": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenStudier til Sentry-kontoen din.
...
```json title="openstudier.json" {4-7}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="openstudier.json" {7-9}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenStudier-samtale oppført som en separat økt.
