...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i OpenStudier-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i OpenStudier lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom OpenStudier-konfigurasjonen.
...
OpenStudier kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.openstudier/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `openstudier.json` konfigurasjonsfil:
...
- Globalt: `~/.config/openstudier/modes/`
- Prosjekt: `.openstudier/modes/`
...
Hvis ingen temperatur er spesifisert, bruker OpenStudier modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenStudier-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.openstudier/modes/` for prosjektspesifikke moduser eller `~/.config/openstudier/modes/` for globale moduser:
