...
description: Skriv dine egne programtillegg for å utvide OpenStudier.
---

Plugins lar deg utvide OpenStudier ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenStudier.
...
- `.openstudier/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openstudier/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openstudier/openstudier.json`)
2. Prosjektkonfigurasjon (`openstudier.json`)
3. Global plugin-katalog (`~/.config/openstudier/plugins/`)
4. Prosjektpluginkatalog (`.openstudier/plugins/`)
...
OpenStudier kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenStudier SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenStudier.
...
:::note
Hvis du bruker OpenStudier-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenStudier fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenStudier:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenStudier kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenStudier sammen med innebygde verktøy.
