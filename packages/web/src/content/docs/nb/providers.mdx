---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenStudier.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenStudier bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenStudier-konfigurasjonen.
...
i `~/.local/share/openstudier/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenStudier
konfig.
...
OpenStudier Zen er en liste over modeller levert av OpenStudier-teamet som har vært
testet og verifisert for å fungere godt med OpenStudier. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenStudier Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenStudier Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i OpenStudier og er helt valgfritt å bruke.

---

## OpenStudier Go

OpenStudier Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenStudier-teamet som har vært
testet og verifisert for å fungere godt med OpenStudier.

1. Kjør kommandoen `/connect` i TUI, velg `OpenStudier Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med OpenStudier:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenStudier:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openstudier.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openstudier støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenStudier skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenStudier-konfigurasjonen.
...
for å lagre token i OpenStudier auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenStudier bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenStudier
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openstudier.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `openstudier.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenStudier:
...
Slik bruker du Google Vertex AI med OpenStudier:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenStudier.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenStudier, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) plugin for automatisk å logge hver OpenStudier-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenStudier-samtale oppført som en separat økt.
...
Du kan konfigurere OpenStudier for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenStudier for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenStudier for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenStudier. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openstudier) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenStudier:
...
3. Kopier API-nøkkelen for bruk i OpenStudier.
...
6. **Viktig**: Før du bruker skymodeller i OpenStudier, må du hente modellinformasjonen lokalt:
...
OpenStudier Zen er en liste over testede og verifiserte modeller levert av OpenStudier-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenStudier Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenStudier API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenStudier-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenStudier-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenStudier:
...
Du kan også tilpasse modeller gjennom OpenStudier-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenStudier. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openstudier.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenStudier forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openstudier auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenStudier-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenStudier-konfigurasjonen.
