---
title: Tjener
description: Samhandle med OpenStudier-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openstudier serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenStudier-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openstudier.local` |
...
Sett `OPENSTUDIER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openstudier` som standard, eller sett `OPENSTUDIER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openstudier serve` og `openstudier web`.
...
Når du kjører `openstudier` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenStudier-serveren til å samhandle med OpenStudier programmatisk.
:::

Denne arkitekturen lar OpenStudier støtte flere klienter og lar deg samhandle med OpenStudier programmatisk.

Du kan kjøre `openstudier serve` for å starte en frittstående server. Hvis du har
OpenStudier TUI kjører, vil `openstudier serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenStudier [IDE](/docs/ide) plugins.
...
openstudier-serveren viser følgende APIer.
