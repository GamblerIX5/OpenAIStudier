...
Med OpenStudier kan du velge fra ett av flere innebygde temaer, bruke et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruker OpenStudier vårt eget `openstudier`-tema.
...
OpenStudier støtter et fleksibelt JSON-basert temasystem som lar brukere enkelt lage og tilpasse temaer.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/openstudier/themes/*.json` eller `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openstudier/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.openstudier/themes/*.json`
