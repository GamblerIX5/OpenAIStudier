...
Verktøy lar LLM utføre handlinger i kodebasen din. OpenStudier kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i OpenStudier.
...
:::note
Dette verktøyet er bare tilgjengelig når `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENSTUDIER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenStudier-leverandøren eller når miljøvariabelen `OPENSTUDIER_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter OpenStudier:
