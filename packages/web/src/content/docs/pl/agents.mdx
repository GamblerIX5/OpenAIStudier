...
W OpenStudier obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenStudier zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenStudier ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenStudier ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `openstudier.json`:
...
- Globalnie: `~/.config/openstudier/agents/`
- W projekcie: `.openstudier/agents/`
...
Jeśli nie zostanie skonfigurowana, OpenStudier używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenStudier, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenStudier w `provider/model-id`. Na przykład, [OpenStudier Zen](/docs/zen), użyj `openstudier/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `openstudier models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/GamblerIX/OpenStudier).
:::
