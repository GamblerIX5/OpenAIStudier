---
title: Konfiguracja
description: Ustawienia OpenStudier w pliku JSON.
---

Możesz dostosować OpenStudier za pomocą pliku konfiguracyjnego JSON.

---

## Format

OpenStudier obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "openstudier"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/openstudier`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/openstudier/openstudier.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENSTUDIER_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`openstudier.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.openstudier`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENSTUDIER_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.openstudier` i `~/.config/openstudier` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/openstudier`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenStudier w `~/.config/openstudier/openstudier.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openstudier/tui.json`.
...
Dodaj `openstudier.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy OpenStudier się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENSTUDIER_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENSTUDIER_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openstudier` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openstudier`, więc **może być łączony** z ich zawartością.
...
Użyj `OPENSTUDIER_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `openstudier.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `openstudier serve` i `openstudier web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer OpenStudier.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `openstudier.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie OpenStudier wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openstudier/agents/` lub `.openstudier/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, OpenStudier powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`openstudier run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openstudier/commands/` lub `.openstudier/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
OpenStudier może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy OpenStudier nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie OpenStudier **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają OpenStudier o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.openstudier/plugins/` lub `~/.config/openstudier/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie OpenStudier tylko do określonych dostawców.
