...
OpenStudier Go to tania subskrypcja za **10 USD miesięcznie**, która zapewnia niezawodny dostęp do popularnych otwartych modeli kodowania.

:::note
OpenStudier Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w OpenStudier. Subskrybujesz OpenStudier Go i otrzymujesz swój klucz API. Jest to **całkowicie opcjonalne** i nie musisz z tego korzystać, aby używać OpenStudier.
...
OpenStudier Go daje dostęp do tych modeli za **10 USD miesięcznie**.

---

## Jak to działa

OpenStudier Go działa jak każdy inny dostawca w OpenStudier.

1. Logujesz się do **<a href={console}>OpenStudier Zen</a>**, subskrybujesz Go i kopiujesz swój klucz API.
2. Uruchamiasz polecenie `/connect` w TUI, wybierasz `OpenStudier Go` i wklejasz swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych przez Go.

:::note
Tylko jeden członek na obszar roboczy może subskrybować OpenStudier Go.
:::
...
OpenStudier Go obejmuje następujące limity:
...
OpenStudier Go to plan subskrypcji za **10 USD miesięcznie**. Poniżej znajdują się ceny **za 1 mln tokenów**.
...
[Identyfikator modelu](/docs/config/#models) w twojej konfiguracji OpenStudier używa formatu `openstudier-go/<model-id>`. Na przykład dla Kimi K2.5 użyłbyś `openstudier-go/kimi-k2.5` w swojej konfiguracji.
...
Stworzyliśmy OpenStudier Go, aby:
...
4. Nie wprowadzać **żadnych blokad (lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy w OpenStudier.
