...
Możesz dodać dodatkowe informacje do openstudier za pomocą _Model Context Protocol_, w skrócie MCP. openstudier obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openstudier Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openstudier`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openstudier automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openstudier:
...
Jeśli serwer wymaga uwierzytelnienia, openstudier poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openstudier mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openstudier bezpieczne przechowa tokeny w `~/.local/share/openstudier/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openstudier, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openstudier, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openstudier z kontem Sentry.
