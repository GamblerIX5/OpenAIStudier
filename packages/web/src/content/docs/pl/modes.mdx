...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji openstudier.
...
te lub skonfiguruj własne za pomocą konfiguracji openstudier.
...
openstudier ma dwa puste tryby.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.openstudier/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `openstudier.json`:
...
- Globalnie: `~/.config/openstudier/modes/`
...
Jeśli nie ma wpływu na temperaturę, openstudier stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna openstudier, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.openstudier/modes/` dla trybów zapisanych dla projektu lub `~/.config/openstudier/modes/` dla trybów globalnych:
