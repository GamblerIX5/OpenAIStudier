---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openstudier.
---

Wtyczki umożliwiające wyłączenie openstudier poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openstudier.
...
- `.openstudier/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openstudier/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openstudier/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openstudier/openstudier.json`)
2. Konfiguracja projektu (`openstudier.json`)
3. Globalny katalog wtyczek (`~/.config/openstudier/plugins/`)
4. Katalog wtyczek projektu (`.openstudier/plugins/`)
...
openstudier uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openstudier do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openstudier.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openstudier, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openstudier czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openstudier:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openstudier. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
