---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openstudier.
---

import config from "../../../../config.mjs"
export const console = config.console

openstudier używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openstudier.
...
w `~/.local/share/openstudier/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openstudier
config.
...
OpenStudier Zen to lista modeli dostarczonych przez zespół openstudier, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openstudier. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenStudier Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenStudier Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w openstudier i jest całkowicie opcjonalny w użyciu.

---

## OpenStudier Go

OpenStudier Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openstudier, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openstudier.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenStudier Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z openstudier:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openstudier:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openstudier.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openstudier nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openstudier działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openstudier.
...
openstudier używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openstudier
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openstudier.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `openstudier.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openstudier:
...
Aby używać Google Vertex AI z openstudier:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openstudier.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openstudier, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session), aby automatycznie rejestrować każdą konwersację openstudier jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openstudier będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openstudier tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openstudier tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openstudier tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openstudier. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openstudier).
:::
...
Aby korzystać z Ollama Cloud z openstudier:
...
3. Skopiuj klucz API do użycia w openstudier.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openstudier musisz pobrać informacje o modelu lokalnie:
...
OpenStudier Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openstudier. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenStudier Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openstudier.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openstudier.
...
5. Możesz je także dostosować za pomocą konfiguracji openstudier. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openstudier:
...
Możesz także dostosować modele za pomocą konfiguracji openstudier. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openstudier. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openstudier.json` w katalogu projektu:
...
Pola `limit` pozwalają openstudier zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openstudier auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openstudier i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openstudier.
