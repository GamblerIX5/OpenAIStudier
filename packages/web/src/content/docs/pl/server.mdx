---
title: Serwer
description: Interakcja z serwerem openstudier za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openstudier serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openstudier.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openstudier.local` |
...
Ustaw `OPENSTUDIER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openstudier` lub ustaw `OPENSTUDIER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openstudier serve`, jak i `openstudier web`.
...
Kiedy uruchomisz `openstudier`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openstudier do programowej interakcji z openstudier.
:::

Ta architektura umożliwia obsługę wielu klientów przez openstudier i programową interakcję z kodem otwartym.

Możesz uruchomić `openstudier serve`, aby uruchomić samodzielny serwer. Jeśli masz
openstudier TUI działa, `openstudier serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openstudier [IDE](/docs/ide).
...
Serwer openstudier udostępnia następujące interfejsy API.
