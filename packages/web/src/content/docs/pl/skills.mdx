...
Umiejętności agenta pozwalają openstudier odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openstudier przeszukuje te lokalizacje:

- Project config: `.openstudier/skills/<name>/SKILL.md`
- Global config: `~/.config/openstudier/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openstudier przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openstudier/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openstudier/skills/git-release/SKILL.md` w ten sposób:
...
openstudier wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openstudier.json`:
...
**Dla agentów wbudowanych** (w `openstudier.json`):
