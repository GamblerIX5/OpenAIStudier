...
Dzięki openstudier możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openstudier używa naszego własnego motywu `openstudier`.
...
openstudier ma kilka wbudowanych motywów.
...
- Chcą, aby OpenStudier pasował do wyglądu ich terminala
...
openstudier obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openstudier/themes/*.json` lub `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openstudier/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openstudier/themes/*.json`
