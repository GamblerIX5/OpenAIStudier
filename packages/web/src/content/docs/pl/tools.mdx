...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenStudier posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenStudier.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENSTUDIER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenStudier lub gdy zmienna środowiskowa `OPENSTUDIER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenStudier:
