...
Aby debugować problemy z openstudier, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openstudier\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `openstudier --log-level DEBUG`.

---

## Przechowywanie danych

openstudier przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openstudier`
...
openstudier Desktop uruchamia w tle lokalny serwer openstudier (sidecar `openstudier-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenStudier` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (lub `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
openstudier może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openstudier\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby openstudier mógł ją odbudować.

1. Całkowicie zamknij openstudier Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openstudier`
- **Linux**: usuń `~/.cache/openstudier` (lub uruchom `rm -rf ~/.cache/openstudier`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\openstudier`

3. Uruchom ponownie openstudier Desktop.

---

### Napraw problemy z połączeniem z serwerem

openstudier Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `openstudier.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENSTUDIER_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENSTUDIER_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows openstudier Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami openstudier.

---

### Brak powiadomień

openstudier Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla openstudier w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie openstudier Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji openstudier Desktop):

- `openstudier.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `openstudier.global.dat` i `openstudier.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z openstudier:
...
3. Upewnij się, że masz najnowszą wersję z `openstudier upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `openstudier models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\openstudier`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. openstudier dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\openstudier`

2. Uruchom ponownie kod openstudier, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to openstudier do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
openstudier wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
