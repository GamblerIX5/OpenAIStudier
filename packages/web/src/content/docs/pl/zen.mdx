---
title: Zen
description: Wyselekcjonowana lista modeli dostarczonych przez OpenStudier.
---
...
OpenStudier Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół OpenStudier.

:::note
OpenStudier Zen jest obecnie w wersji beta.
:::

Zen działa jak każdy inny dostawca w OpenStudier. Logujesz się do OpenStudier Zen i otrzymujesz
...
:::tip
Przetestowaliśmy wybraną grupę modeli i dostawców, którzy dobrze współpracują z OpenStudier.
:::
...
OpenStudier Zen to brama AI, która zapewnia dostęp do tych modeli.

---

## Jak to działa

OpenStudier Zen działa jak każdy inny dostawca w OpenStudier.

1. Logujesz się do **<a href={console}>OpenStudier Zen</a>**, dodajesz dane rozliczeniowe
   i kopiujesz swój klucz API.
2. Uruchamiasz polecenie `/connect` w TUI, wybierasz OpenStudier Zen i wklejasz klucz API.
...
[Identyfikator modelu](/docs/config/#models) w konfiguracji OpenStudier
używa formatu `openstudier/<model-id>`. Na przykład w przypadku GPT 5.2 Codex użyłbyś
`openstudier/gpt-5.2-codex` w swojej konfiguracji.
...
- MiniMax M2.5 Free jest dostępny w OpenStudier przez ograniczony czas. Zespół wykorzystuje ten czas na zbieranie opinii i ulepszanie modelu.
- Big Pickle to ukryty model, który jest bezpłatny w OpenStudier przez ograniczony czas. Zespół wykorzystuje ten czas na zbieranie opinii i ulepszanie modelu.
...
Stworzyliśmy OpenStudier Zen, aby:
...
4. Nie **mieć blokady** (no lock-in), umożliwiając używanie go z dowolnym innym agentem kodującym. I zawsze pozwalać na korzystanie z dowolnego innego dostawcy w OpenStudier.
