---
title: Providers
description: Using any LLM provider in OpenStudier.
---

import config from "../../../config.mjs"
export const console = config.console

OpenStudier uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenStudier config.
...
You can customize the providers through the `provider` section in your OpenStudier
config.
...
OpenStudier Zen is a list of models provided by the OpenStudier team that have been
tested and verified to work well with OpenStudier. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenStudier Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenStudier Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in OpenStudier and is completely optional to use.

---

## OpenStudier Go

OpenStudier Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenStudier team that have been
tested and verified to work well with OpenStudier.

1. Run the `/connect` command in the TUI, select `OpenStudier Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with OpenStudier:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running openstudier:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `openstudier.json`:
...
:::info
Using your Claude Pro/Max subscription in OpenStudier is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for openstudier to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your openstudier config.
...
OpenStudier uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenStudier
to only use your own GitLab-hosted instance, add the following to your
`openstudier.json` file. It is also recommended to disable session sharing.
...
More documentation on [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with openstudier:
...
To use Google Vertex AI with OpenStudier:
...
2. Set the required environment variables:
   ...
   Set them while running openstudier.
...
In the event you see a feature or model from Helicone that isn't configured automatically through openstudier, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) plugin to automatically log each OpenStudier conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenStudier conversation listed as a separate session.
...
You can configure openstudier to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure openstudier to use local models through LM Studio.
...
You can configure openstudier to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenStudier. See the [Ollama integration docs](https://docs.ollama.com/integrations/openstudier) for details.
:::
...
To use Ollama Cloud with OpenStudier:
...
3. Copy the API key for use in OpenStudier.
...
6. **Important**: Before using cloud models in OpenStudier, you must pull the model information locally:
...
OpenStudier Zen is a list of tested and verified models provided by the OpenStudier team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenStudier Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenStudier API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your openstudier config.
...
5. You can also customize them through your openstudier config. Here's an example of specifying a provider
...
You can also customize models through your openstudier config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with openstudier. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `openstudier.json` file in your project directory:
...
The `limit` fields allow OpenStudier to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `openstudier auth list` to see if the credentials
...
2. For custom providers, check the openstudier config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your openstudier config.
