...
Existem dois tipos de agentes no openstudier; agentes primários e subagentes.
...
openstudier vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
openstudier vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

openstudier vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `openstudier.json`:
...
- Por projeto: `.openstudier/agents/`
...
Se nenhuma temperatura for especificada, o openstudier usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openstudier quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openstudier usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenStudier Zen](/docs/zen), você usaria `openstudier/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `openstudier models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/GamblerIX/OpenStudier).
:::
