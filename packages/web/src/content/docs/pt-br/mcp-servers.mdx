...
Você pode adicionar ferramentas externas ao openstudier usando o _Model Context Protocol_, ou MCP. O openstudier suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do openstudier](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/openstudier`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O openstudier lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o openstudier irá:
...
Se o servidor requer autenticação, o openstudier solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `openstudier mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o openstudier armazenará os tokens de forma segura em `~/.local/share/openstudier/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no openstudier, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do openstudier como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o openstudier à sua conta do Sentry.
