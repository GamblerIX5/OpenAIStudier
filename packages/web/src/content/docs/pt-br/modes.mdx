...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do openstudier. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no openstudier permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do openstudier.
...
O openstudier vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.openstudier/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `openstudier.json`:
...
- Global: `~/.config/openstudier/modes/`
- Projeto: `.openstudier/modes/`

```markdown title="~/.config/openstudier/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o openstudier usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openstudier quanto para a configuração específica do projeto.
...
```json title="openstudier.json" {4-14}
{
  "$schema": "https://opencode.ai/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.openstudier/modes/` para modos específicos do projeto ou `~/.config/openstudier/modes/` para modos globais:

```markdown title=".openstudier/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/openstudier/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
