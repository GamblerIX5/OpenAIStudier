---
title: Plugins
description: Escreva seus próprios plugins para estender o openstudier.
---

Plugins permitem que você estenda o openstudier conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openstudier.
...
- `.openstudier/plugins/` - Plugins em nível de projeto
- `~/.config/openstudier/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openstudier/node_modules/`.
...
1. Configuração global (`~/.config/openstudier/openstudier.json`)
2. Configuração do projeto (`openstudier.json`)
3. Diretório de plugins global (`~/.config/openstudier/plugins/`)
4. Diretório de plugins do projeto (`.openstudier/plugins/`)
...
O openstudier executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openstudier para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openstudier.
...
:::note
Se você estiver usando o aplicativo desktop openstudier, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openstudier de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openstudier:
...
O helper `tool` cria uma ferramenta personalizada que o openstudier pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openstudier junto com as ferramentas integradas.
