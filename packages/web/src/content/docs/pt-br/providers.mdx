---
title: Provedores
description: Usando qualquer provedor LLM no openstudier.
---

import config from "../../../../config.mjs"
export const console = config.console

openstudier usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do openstudier.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/openstudier/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do openstudier.
...
OpenStudier Zen é uma lista de modelos fornecidos pela equipe do openstudier que foram testados e verificados para funcionar bem com o openstudier. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenStudier Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenStudier Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no openstudier e é completamente opcional.

---

## OpenStudier Go

OpenStudier Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do openstudier que foram
testados e verificados para funcionar bem com o openstudier.

1. Execute o comando `/connect` no TUI, selecione `OpenStudier Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Para usar o Amazon Bedrock com o openstudier:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o openstudier:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `openstudier.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no openstudier não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o openstudier funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do openstudier.
...
2. Escolha seu método de autenticação:
   ...
   3. Nome: `OpenStudier`, Escopos: `api`
...
armazenar o token no armazenamento de autenticação do openstudier.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
openstudier usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o openstudier
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `openstudier.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuração

Personalize através do `openstudier.json`:
...
Para usar sua assinatura do GitHub Copilot com o openstudier:
...
Para usar o Google Vertex AI com o openstudier:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o openstudier.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do openstudier, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) para registrar automaticamente cada conversa do openstudier como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do openstudier listada como uma sessão separada.
...
Você pode configurar o openstudier para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o openstudier para usar modelos locais através do LM Studio.
...
Você pode configurar o openstudier para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o openstudier. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/openstudier) para detalhes.
:::
...
Para usar o Ollama Cloud com o openstudier:
...
3. Copie a chave da API para uso no openstudier.
...
6. **Importante**: Antes de usar modelos em nuvem no openstudier, você deve puxar as informações do modelo localmente:
...
OpenStudier Zen é uma lista de modelos testados e verificados fornecidos pela equipe do openstudier. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>OpenStudier Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **OpenStudier Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do openstudier.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do openstudier.
...
5. Você também pode personalizá-los através da sua configuração do openstudier. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o openstudier:
...
Você também pode personalizar modelos através da sua configuração do openstudier. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o openstudier. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no openstudier.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `openstudier.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o openstudier entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `openstudier auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do openstudier e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do openstudier.
