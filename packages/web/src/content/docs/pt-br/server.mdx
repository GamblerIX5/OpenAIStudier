---
title: Servidor
description: Interaja com o servidor openstudier via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openstudier serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openstudier pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openstudier.local` |
...
Defina `OPENSTUDIER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openstudier`, ou defina `OPENSTUDIER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openstudier serve` quanto ao `openstudier web`.

```bash
OPENSTUDIER_SERVER_PASSWORD=your-password openstudier serve
```
...
Quando você executa `openstudier`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openstudier para interagir com o openstudier programaticamente.
:::

Essa arquitetura permite que o openstudier suporte múltiplos clientes e permite que você interaja com o openstudier programaticamente.

Você pode executar `openstudier serve` para iniciar um servidor autônomo. Se você tiver o TUI do openstudier em execução, `openstudier serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openstudier [IDE](/docs/ide).
...
O servidor openstudier expõe as seguintes APIs.
