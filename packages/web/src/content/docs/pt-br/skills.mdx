...
As habilidades do agente permitem que o openstudier descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openstudier pesquisa nesses locais:

- Configuração do projeto: `.openstudier/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openstudier/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openstudier sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openstudier/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openstudier/skills/git-release/SKILL.md` assim:
...
O openstudier lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openstudier.json`:
...
**Para agentes embutidos** (em `openstudier.json`):
