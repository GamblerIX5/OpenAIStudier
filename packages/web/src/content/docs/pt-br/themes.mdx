...
Com o openstudier, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openstudier usa nosso próprio tema `openstudier`.
...
O openstudier vem com vários temas embutidos.
...
- Querem que o openstudier corresponda à aparência do seu terminal
...
O openstudier suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openstudier/themes/*.json` ou `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openstudier/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openstudier/themes/*.json`
