...
Para depurar problemas com o openstudier, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openstudier\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `openstudier --log-level DEBUG`.

---

## Armazenamento

O openstudier armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/openstudier/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\openstudier`
...
O openstudier Desktop executa um servidor openstudier local (o sidecar `openstudier-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `OpenStudier` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/openstudier/openstudier.jsonc` (ou `~/.config/openstudier/openstudier.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/openstudier/openstudier.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openstudier\openstudier.jsonc`
...
O openstudier também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/openstudier/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\openstudier\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o openstudier possa reconstruí-lo.

1. Saia completamente do openstudier Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/openstudier`
- **Linux**: exclua `~/.cache/openstudier` (ou execute `rm -rf ~/.cache/openstudier`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\openstudier`

3. Reinicie o openstudier Desktop.

---

### Corrigindo problemas de conexão com o servidor

O openstudier Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `openstudier.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `OPENSTUDIER_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `OPENSTUDIER_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o openstudier Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do openstudier.

---

### Notificações não aparecendo

O openstudier Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o openstudier nas configurações do seu sistema operacional, e
...
1. Saia do openstudier Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo openstudier Desktop):

- `openstudier.settings.dat` (URL do servidor padrão do desktop)
- `openstudier.global.dat` e `openstudier.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o openstudier:
...
Para descobrir quais modelos você tem acesso, execute `openstudier models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\openstudier`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O openstudier instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\openstudier`

2. Reinicie o openstudier para reinstalar os pacotes de provedores mais recentes

Isso forçará o openstudier a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O openstudier detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
