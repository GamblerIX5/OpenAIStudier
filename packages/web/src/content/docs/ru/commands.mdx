...
Создайте `.openstudier/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openstudier или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openstudier [config](/docs/config):
...
- Глобальный: `~/.config/openstudier/commands/`
- Для каждого проекта: `.openstudier/commands/`
...
openstudier включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
