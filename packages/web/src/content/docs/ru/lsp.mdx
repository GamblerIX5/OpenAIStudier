---
title: LSP-серверы
description: openstudier интегрируется с вашими серверами LSP.
---

openstudier интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

openstudier поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `OPENSTUDIER_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда openstudier открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации openstudier.
