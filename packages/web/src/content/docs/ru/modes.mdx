...
:::caution
Режимы теперь настраиваются с помощью опции `agent` в конфигурации openstudier.
Опция `mode` устарела. [Подробнее ](/docs/agents).
:::

Режимы в openstudier позволяют настраивать поведение, инструменты и подсказки для различных вариантов использования.

Он имеет два встроенных режима: **сборка** и **планирование**. Вы можете настроить
эти или настройте свои собственные через конфигурацию openstudier.
...
openstudier имеет два встроенных режима.
...
- `edit` – невозможно изменить существующие файлы, за исключением файлов, расположенных по адресу `.openstudier/plans/*.md`, для детализации самого плана.
...
Настройте режимы в файле конфигурации `openstudier.json`:
...
- Глобальный: `~/.config/openstudier/modes/`
- Проект: `.openstudier/modes/`
...
Если температура не указана, openstudier использует значения по умолчанию для конкретной модели (обычно 0 для большинства моделей, 0,55 для моделей Qwen).
...
как глобальная конфигурация openstudier, так и конфигурация конкретного проекта.
...
Создайте файлы режимов в `.openstudier/modes/` для режимов, специфичных для проекта, или в `~/.config/openstudier/modes/` для глобальных режимов:
