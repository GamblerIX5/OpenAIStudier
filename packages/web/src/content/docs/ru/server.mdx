---
title: Сервер
description: Взаимодействуйте с сервером openstudier через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Команда `openstudier serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| `--mdns-domain` | Пользовательское доменное имя для mDNS      | `openstudier.local` |
...
Установите `OPENSTUDIER_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `openstudier` или установите `OPENSTUDIER_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `openstudier serve`, так и к `openstudier web`.
...
Когда вы запускаете `openstudier`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер openstudier для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `openstudier serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `openstudier serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами openstudier [IDE](/docs/ide).
...
Сервер openstudier предоставляет следующие API.
