---
title: Server
description: Interact with openstudier server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openstudier serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openstudier client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openstudier.local` |
...
Set `OPENSTUDIER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openstudier`, or set `OPENSTUDIER_SERVER_USERNAME` to override it. This applies to both `openstudier serve` and `openstudier web`.
...
When you run `openstudier` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openstudier server to interact with openstudier programmatically.
:::

This architecture lets openstudier support multiple clients and allows you to interact with openstudier programmatically.

You can run `openstudier serve` to start a standalone server. If you have the
openstudier TUI running, `openstudier serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenStudier [IDE](/docs/ide) plugins.
...
The openstudier server exposes the following APIs.
