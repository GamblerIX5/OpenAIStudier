...
With OpenStudier you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenStudier uses our own `openstudier` theme.
...
OpenStudier comes with several built-in themes.
...
- Want OpenStudier to match their terminal's appearance
...
OpenStudier supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openstudier/themes/*.json` or `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openstudier/themes/*.json`
4. **Current working directory** - Defined in `./.openstudier/themes/*.json`
