...
openstudier'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
openstudier, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
openstudier, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

openstudier iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `openstudier.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.openstudier/agents/`
...
Sıcaklık belirtilmezse openstudier modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global openstudier yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
openstudier yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenStudier Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `openstudier/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `openstudier models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/GamblerIX/OpenStudier).
:::
