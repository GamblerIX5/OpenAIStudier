...
OpenStudier Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
OpenStudier Go şu anda beta aşamasındadır.
:::

Go, OpenStudier içindeki diğer sağlayıcılar gibi çalışır. OpenStudier Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenStudier'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
OpenStudier ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
OpenStudier Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

OpenStudier Go, OpenStudier'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>OpenStudier Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenStudier Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanılabilen modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Çalışma alanı başına yalnızca bir üye OpenStudier Go'ya abone olabilir.
:::
...
OpenStudier Go aşağıdaki sınırları içerir:
...
OpenStudier Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
OpenStudier yapılandırmanızdaki [model kimliği](/docs/config/#models), `openstudier-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `openstudier-go/kimi-k2.5` kullanırsınız.
...
OpenStudier Go'yu şu amaçlarla oluşturduk:
...
4. OpenStudier ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.
