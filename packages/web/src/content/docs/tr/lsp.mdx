...
description: OpenStudier, LSP sunucularınızla bütünleşir.
---

OpenStudier, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

OpenStudier, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OPENSTUDIER_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
openstudier bir dosyayı açtığında:
...
LSP sunucularını, openstudier yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
