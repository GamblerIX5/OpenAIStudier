...
_Model Bağlam Protokolü_ veya MCP kullanarak openstudier'a harici araçlar ekleyebilirsiniz. openstudier hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openstudier Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openstudier` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openstudier, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openstudier şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openstudier, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openstudier mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openstudier, belirteçleri `~/.local/share/openstudier/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openstudier'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openstudier yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openstudier'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
