...
:::caution
Modlar artık openstudier yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

openstudier'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya openstudier yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
openstudier iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.openstudier/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`openstudier.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, openstudier modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global openstudier yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.openstudier/modes/` veya genel modlar için `~/.config/openstudier/modes/`'de mod dosyaları oluşturun:
