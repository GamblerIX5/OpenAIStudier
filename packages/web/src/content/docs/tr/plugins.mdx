---
title: Eklentiler
description: OpenStudier'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenStudier'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenStudier'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openstudier/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openstudier/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openstudier/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openstudier/openstudier.json`)
2. Project config (`openstudier.json`)
3. Global eklenti dizini (`~/.config/openstudier/plugins/`)
4. Proje eklenti dizini (`.openstudier/plugins/`)
...
OpenStudier bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenStudier SDK istemcisi.
...
OpenStudier'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenStudier masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenStudier'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenStudier'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenStudier'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenStudier'a da hazır olacaktır.
