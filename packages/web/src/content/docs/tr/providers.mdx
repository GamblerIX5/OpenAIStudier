---
title: Sağlayıcılar
description: openstudier'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openstudier, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openstudier yapılandırmanızda yapılandırın.
...
`~/.local/share/openstudier/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openstudier'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenStudier Zen, openstudier ekibi tarafından sağlanan modellerin bir listesidir.
openstudier ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenStudier Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, openstudier'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
openstudier'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenStudier Go

OpenStudier Go, openstudier ile iyi çalıştığı test edilmiş ve doğrulanmış, openstudier ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenStudier Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u openstudier ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openstudier'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openstudier.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi openstudier'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openstudier'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in openstudier auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
openstudier, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openstudier'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`openstudier.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [@gitlab/opencode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`openstudier.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi openstudier'la kullanmak için:
...
Google Vertex AI'yi openstudier ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openstudier çalıştırılırken ayarlayın.
...
Helicone'un openstudier aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openstudier görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [openstudier-helicone-session](https://github.com/H2Shami/openstudier-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openstudier görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openstudier'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openstudier'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openstudier'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openstudier için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openstudier)'a bakın.
:::
...
Ollama Cloud'u openstudier ile kullanmak için:
...
3. openstudier'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openstudier'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenStudier Zen, openstudier ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenStudier Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenStudier Zen**'i arayın.

   ```txt
   /connect
   ```

3. openstudier API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openstudier yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openstudier yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri openstudier yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openstudier ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `openstudier.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları openstudier'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openstudier auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openstudier yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openstudier yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
