...
Ajan becerileri, openstudier'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openstudier su konumlari tarar:

- Proje konfigurasyonu: `.openstudier/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openstudier/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openstudier, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openstudier/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openstudier/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openstudier/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openstudier, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openstudier.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openstudier.json` icinde):
