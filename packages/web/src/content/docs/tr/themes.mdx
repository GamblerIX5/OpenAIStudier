...
openstudier ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openstudier kendi `openstudier` temasını kullanır.
...
openstudier birden fazla yerleşik temayla gelir.
...
- openstudier'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openstudier, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openstudier/themes/*.json` veya `$XDG_CONFIG_HOME/openstudier/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openstudier/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openstudier/themes/*.json`
