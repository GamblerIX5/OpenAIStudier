...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openstudier yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openstudier içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENSTUDIER_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENSTUDIER_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openstudier provider kullanılırken veya `OPENSTUDIER_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openstudier başlatırken etkinleştirmek için:
