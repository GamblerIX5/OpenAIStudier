---
title: TUI
description: OpenStudier terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

OpenStudier'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
OpenStudier TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
OpenStudier'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
OpenStudier'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `openstudier.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `OPENSTUDIER_TUI_CONFIG` kullanın.
