---
title: Zen
description: openstudier tarafından sağlanan seçilmiş modeller listesi.
---
...
OpenStudier Zen, openstudier ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
OpenStudier Zen şu anda beta aşamasındadır.
:::

Zen, openstudier'daki diğer sağlayıcılar gibi çalışır. OpenStudier Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve openstudier kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
openstudier ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
OpenStudier Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

OpenStudier Zen, openstudier'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenStudier Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, OpenStudier Zen'i seçin ve API anahtarınızı yapıştırın.
...
openstudier yapılandırmanızdaki [model kimliği](/docs/config/#models) `openstudier/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `openstudier/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için OpenStudier'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için OpenStudier'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
OpenStudier Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman OpenStudier ile başka bir sağlayıcıyı kullanmanıza izin vermek.
