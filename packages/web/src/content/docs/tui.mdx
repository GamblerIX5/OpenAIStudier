---
title: TUI
description: Using the OpenStudier terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenStudier starts the TUI for the current directory.
...
When using the OpenStudier TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenStudier. Allows you to select from available providers and add their API keys.
...
Exit OpenStudier. _Aliases_: `/quit`, `/q`
...
This is separate from `openstudier.json`, which configures server/runtime behavior.
...
Use `OPENSTUDIER_TUI_CONFIG` to load a custom TUI config path.
