...
创建 `.openstudier/commands/test.md`：
...
你可以通过 OpenStudier 配置或在 `commands/` 目录中创建 markdown 文件来添加自定义命令。

---

### JSON

在 OpenStudier [配置](/docs/config)中使用 `command` 选项：
...
- 全局：`~/.config/openstudier/commands/`
- 项目级：`.openstudier/commands/`
...
openstudier 包含多个内置命令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`；[了解更多](/docs/tui#commands)。
