...
OpenStudier Go 是一項低成本的 **每月 10 美元** 訂閱服務，讓您可以穩定存取熱門的開放原始碼程式設計模型。

:::note
OpenStudier Go 目前處於測試階段 (Beta)。
:::

Go 的運作方式就像 OpenStudier 中的任何其他供應商一樣。您訂閱 OpenStudier Go 並取得您的 API key。這是**完全選用**的，您不需要使用它也能使用 OpenStudier。
...
:::tip
我們測試了一組精選的模型和供應商，它們與 OpenStudier 搭配運作良好。
:::
...
OpenStudier Go 讓您能以 **每月 10 美元** 的價格存取這些模型。

---

## 運作方式

OpenStudier Go 的運作方式就像 OpenStudier 中的任何其他供應商一樣。

1. 您登入 **<a href={console}>OpenStudier Zen</a>**，訂閱 Go，並複製您的 API key。
2. 您在 TUI 中執行 `/connect` 指令，選擇 `OpenStudier Go`，並貼上您的 API key。
3. 在 TUI 中執行 `/models` 以查看透過 Go 可用的模型清單。

:::note
每個工作區只能有一位成員訂閱 OpenStudier Go。
:::
...
OpenStudier Go 是一個 **每月 10 美元** 的訂閱方案。以下是 **每 100 萬 token** 的價格。
...
您 OpenStudier 設定中的 [model id](/docs/config/#models) 使用 `openstudier-go/<model-id>` 格式。例如，對於 Kimi K2.5，您會在設定中使用 `openstudier-go/kimi-k2.5`。
