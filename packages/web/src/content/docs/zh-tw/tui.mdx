---
title: TUI
description: 使用 OpenStudier 終端機使用者介面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenStudier 提供了一個互動式終端機介面（TUI），用於配合 LLM 處理您的專案。

執行 OpenStudier 即可啟動當前目錄的 TUI。
...
使用 OpenStudier TUI 時，您可以輸入 `/` 後跟指令名稱來快速執行操作。例如：
...
將供應商新增到 OpenStudier。允許您從可用的供應商中選擇並新增其 API 金鑰。
...
這與設定伺服器/執行時行為的 `openstudier.json` 是分開的。
...
使用 `OPENSTUDIER_TUI_CONFIG` 載入自訂 TUI 設定路徑。
