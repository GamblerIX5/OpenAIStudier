...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with openstudier...`)
  const prompt = [
  ...
  try {
    await $`openstudier run -m openstudier/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  openstudier failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with openstudier")
  return true
}
